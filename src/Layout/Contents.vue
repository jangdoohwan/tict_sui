...
export default {
  ...
  watch: {
    $route() {
      ...
      // iframe 페이지인 경우
      if (routePath === '/page') {
        routePath = this.$route.params.url
      }
      this.menu =
        this.$store.state.menu.menus.find((m) => m.href === routePath) || {}
    },
    menu() {
      this.setLocation()
    }
    ...
    setLocation() {
      ...
      if (Object.keys(this.menu).length > 0) {
        ...
        while (menu != null && menu.prnt_menu_id !== '#') {
          menu = this.$store.state.menu.menus.find(
            (m) => m.menu_id === menu.prnt_menu_id
          )
          location.push(menu)
        }
