  ...
  <div>
    <div class="search-box">
      <div class="left form-inline">
        ...
        <div class="form-group">
          <span class="form-label mx-1">{{ $t('colStateCd') }}</span>
          <select
            ref="statusSearchType"
            v-model="statusSearchType"
            class="form-control"
            title="수집 상태"
          >
            <option
              v-for="(item, index) in colStateCdList"
              :key="'item' + index"
              :value="item.value"
              >{{ item.name }}</option
            >
        ...
        <!-- 기간검색 -->
        <div class="form-group">
          <span class="form-label mx-1">수집일자</span>
          <datetime-picker
            :isPair="true"
        ...
        <div class="form-group">
          <button class="btn btn-info" type="button" @click="fnSearchList()">
            {{ $t('search') }}
          </button>
          <button class="btn btn-info" type="button" @click="searchInit()">
            {{ $t('initializing') }}
          </button>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      colModelList: [
        ...
        { name: 'devMdlNm', index: 'devMdlNm', align: 'left', width: 50 },
        { name: 'devUname', index: 'devUname', align: 'left', width: 50 },
  ...
  methods: {
    ...
    fnGetList() {
      ...
      searchData.startDate = moment(this.startDate).format('YYYYMMDDHHmmss')
      searchData.endDate = moment(this.endDate).format('YYYYMMDDHHmmss')
