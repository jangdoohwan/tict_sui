...
export default {
  ...
  methods: {
    ...
    fnAddSvc() {
      ...
      this.allSvc.selected.forEach((svc) => {
        ...
        if (index === -1) {
          ...
          this.alert(this.$t('exist-svr'), 'warning')
        }
      })
    ...
    fnRemoveSvc() {
      ...
      const usrSvcSelectList = JSON.parse(JSON.stringify(this.usrSvc.selected))
      const userSvcList = JSON.parse(JSON.stringify(this.usrSvc.list))

      usrSvcSelectList.forEach((svc) => {
        // 조직의 서비스는 삭제 불가능하다
        if (svc.svcCd === this.orgSvc.svcCd) {
          this.alert('기본 서비스는 삭제할 수 없습니다.')
        } else {
          let index = userSvcList.findIndex((s) => s.svcCd === svc.svcCd)
          if (index > -1) {
            userSvcList.splice(index, 1)
          }
        }
      })

      this.usrSvc.list = userSvcList
