<template>
  <div class="facility-container">
    <div
      ref="treeWrap"
      class="facility-tree-content md-width-100 width-30 pr-2 md-p-0"
    >
      <div class="facility-tree-title mb-2">
      ...
      <ul class="form-inbox mt-2">
        ...
        <li>
          ...
          <div class="form-group form-inline">
            ...
            />
            <button class="btn btn-gray" @click="fnDuplicate">
              {{ $t('duplication-check') }}
            </button>
          </div>
        </li>
...
export default {
  ...
  methods: {
    ...
    fnGetFacilityInfo(event, target) {
      ...
      this.facility.parentFacSeq =
        this.facility.parentFacSeq === 'ROOT'
          ? null
          : this.facility.parentFacSeq
      ...
      requestApi({
        ...
        callback: res => {
          if (res.data && res.data.count === 0) {
            ...
            this.isCheckDuplicate = false
            if (res.result.status === '600') {
              this.alert(
                this.$t('duplicate-data').replace(
                  '$0',
                  this.$t('facility-name')
                )
              )
            }
          }
    ...
    fnValidateData() {
      ...
      if (
        !this.facility.useYn ||
        (this.facility.useYn && this.facility.useYn === '')
      ) {
