    ...
    <div class="dashboard setting">
      ...
      <div class="right">
        ...
        <div class="setting-btm">
          ...
          <div
            class="templete-setting flex"
            :class="{ 'justify-center': !componentData.tmplRuleImg }"
          >
            <!-- =============== 템플릿 이미지 =============== -->
            <div class="unit flex justify-center items-center flex-col">
              <div
                v-if="componentData.tmplRuleImg"
                class="image-box flex justify-center items-center"
              >
                <img
            ...
            <div class="unit px-0">
              ...
              <div class="form-item">
                <span
                  ...
                  :class="{ 'lb-txt': !noRequired.includes('tmplGubun') }"
                  >{{ $t('tmpl-gubun') }}</span
                >
                <select
              ...
              <div v-if="componentData.tmplGubun === 'T'" class="form-item">
                <span class="form-label w-32 lb-txt">{{ $t('topic') }}</span>
                <select
              ...
              <div v-if="componentData.tmplGubun === 'A'" class="form-item">
                <span class="form-label w-32 lb-txt">{{ $t('api') }}</span>
              ...
              <template v-if="initOptionObj">
                <div
                  ...
                  class="form-item"
                >
                  <span class="form-label w-32 lb-txt">{{ value }}</span>
                  <select
                    v-if="key === 'POPUP'"
                    class="form-control"
                  ...
                  <select
                    v-else-if="key === 'NUM_FORMAT'"
                    class="form-control"
                    v-model="componentData.optionData[key]"
                  >
                    <option selected value="FLOAT">{{ $t('num-float') }}</option>
                    <option value="INT">{{ $t('num-int') }}</option>
                  </select>
                  <input
                    v-else
                    type="text"
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td>
                        ...
                        <select
                          v-if="search.TYPE === 'CALENDAR'"
                          v-model="search.OPTION.format"
                          class="form-control"
                        >
                        ...
                        <template v-else-if="key === 'DEV_MDL'">
                          ...
                          >
                            ...
                            <option
                              v-for="(
                                mdl, mdlIndex
                              ) in fnGetDevMdlListByDevClsCd(data.DEV_CLS)"
                        ...
                        <template v-else-if="key === 'TAG_VALUE_BY_CLS'">
                          <select v-model="data[key]" class="form-control">
                            ...
                            <option
                              v-for="(
                                attb, attbIndex
                              ) in fnGetDeviceAttrListByDevMdlCd(data.DEV_MDL)"
...
const CLICK_TABLE_POPUPS = [
  ...
  { value: 'ProductionLineDetImagePopupSvc', name: '생산라인이상감지 이미지' }
]
...
export default {
  ...
  computed: {
    ...
    noRequired() {
      ...
      let initMsgObj = JSON.parse(this.componentData.initMsg)
      return initMsgObj.NO_REQUIRED ? initMsgObj.NO_REQUIRED : []
    },
    visibleSearchData() {
  ...
  methods: {
    ...
    fnGetComponentData(usrDashTmplSeq) {
      ...
      this.fnInitComponentData()
      this.$refs.templateListComp.fnClearCheckedTemplate()
      ...
      requestApi({
        ...
        callback: (res) => {
          ...
          if (componentData.tmplGubun === 'A') {
            ...
            componentData.apiParamList = componentData.apiParamList.sort(
              (a, b) => {
    ...
    fnChangeSearchDataType(search) {
      ...
      // 검색 타입에 따른 default 컬럼명 세팅
      if (search.TYPE === 'CTN') {
        search.KEY = 'devUname'
      } else if (search.TYPE === 'ORG') {
        search.KEY = 'orgSeq'
    ...
    fnAddRspMsgData() {
      ...
      Object.keys(rspMsgObj).forEach((key) => {
        // 기본값 설정
        if (key === 'NO') {
          // 순번인 경우 index를 기본값으로 설정
          rspMsgObj[key] = this.componentData.rspMsgData.length + 1
        } else if (key === 'COLOR') {
          // 컬러인 경우 컬러 기본값 설정
          const index =
            this.componentData.rspMsgData.length % this.chartColorSet.length
          rspMsgObj[key] = this.chartColorSet[index]
        } else {
          rspMsgObj[key] = ''
        }
      })
    ...
    fnSaveComponent() {
      ...
      let reqData = {
        ...
        devClsCdNm: devCls ? devCls.devClsNm : 'all',
        svcCd: this.componentData.svcCd,
    ...
    fnAddComponent(paramObj) {
      requestApi({
        url: '/online/com/usrDash/insertDashComp',
        headers: {
          'X-MID': this.xMid
        },
        method: 'post',
    ...
    fnModifyComponent(paramObj) {
      requestApi({
        url: '/online/com/usrDash/updateDashComp',
        headers: {
          'X-MID': this.xMid
        },
        method: 'post',
    ...
    fnValidSaveComponent() {
      ...
      // 영상 서비스가 아닌 경우 템플릿 구분 선택 확인
      // 템플릿에 템플릿 구분(Topic, API, Query) no required 설정되지 않은 경우
      if (
        this.componentData.svcCd !== 'SD00000400' &&
        !this.noRequired.includes('tmplGubun') &&
        this.componentData.tmplGubun === ''
      ) {
        this.alert(this.$t('request-tmpl-gubun-select'), 'warning')
        return false
      }
      // 템플릿 구분 API / 선택된 API 없는 경우
      if (
        (this.componentData.tmplGubun === 'A' && !this.componentData.apiUrl) ||
        (this.componentData.tmplGubun === 'A' &&
          this.componentData.apiUrl &&
          this.componentData.apiUrl.trim() === '')
      ) {
        this.alert(this.$t('request-api-search'), 'warning')
        return false
      }
      // 템플릿 구분 TOPIC / 선택된 TOPIC 없는 경우
      if (
      ...
      // 제목 입력하지 않은 경우
      if (
        !this.componentData.dashTmplTitle ||
        (this.componentData.dashTmplTitle &&
          this.componentData.dashTmplTitle.trim() === '')
      ) {
      ...
      // 설명 입력하지 않은 경우
      if (
        !this.componentData.dashTmplDesc ||
        (this.componentData.dashTmplDesc &&
          this.componentData.dashTmplDesc.trim() === '')
      ) {
        this.alert(this.$t('request-description-input'), 'warning')
        return false
      }
      // 템플릿 구분 API / 입력된 API 파라미터 없는 경우
      if (this.componentData.tmplGubun === 'A') {
      ...
      // 템플릿 구분 QUERY / QUERY문 입력 확인
      if (
        (this.componentData.tmplGubun === 'Q' && !this.componentData.query) ||
        (this.componentData.tmplGubun === 'Q' &&
          this.componentData.query &&
          this.componentData.query.trim() === '')
      ) {
        this.alert(this.$t('request-query-input'))
        return false
      }
    ...
    fnDeleteComponent() {
      requestApi({
        url: '/online/com/usrDash/deleteDashComp',
        headers: {
          'X-MID': this.xMid
        },
        method: 'post',
    ...
    // 컴포넌트 상세 템플릿 구분 변경 시 TOPIC / API / QUERY 초기화
    changeTmplGubun() {
      this.componentData.topicId = ''
      this.componentData.apiUrl = ''
      this.componentData.query = ''
