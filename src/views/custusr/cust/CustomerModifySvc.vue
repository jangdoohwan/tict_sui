          ...
          <div v-else>
            <span class="form-data file-add">
              <a
                ...
                @click="
                  fnDownloadLogo(view.uploadedFile.custWlogoFile.fileListSeq)
                "
              >
                {{ view.uploadedFile.custWlogoFile.orgFileName }} [{{
                  view.uploadedFile.custWlogoFile.fileSizeLabel
                }}]
              </a>
            </span>
            <span
              @click="
                fnDeleteFile(
                  view.uploadedFile.custWlogoFile.fileListSeq,
                  'custWlogoFile'
                )
              "
          ...
          <div v-else>
            <span class="form-data file-add">
              <a
                href="javascript:void(0)"
                @click="
                  fnDownloadLogo(view.uploadedFile.custMlogoFile.fileListSeq)
                "
              >
                {{ view.uploadedFile.custMlogoFile.orgFileName }} [{{
                  view.uploadedFile.custMlogoFile.fileSizeLabel
                }}]
              </a>
            </span>
            <span
              @click="
                fnDeleteFile(
                  view.uploadedFile.custMlogoFile.fileListSeq,
                  'custMlogoFile'
                )
              "
              ><a href="#" class="btn ico-delete remove-field"></a
  ...
  methods: {
        ...
        callback: res => {
          if (res.result.status === '200' || res.result.status === '204') {
            ...
            if (res.data.file && res.data.file.length > 0) {
              this.view.uploadedFile.custMlogoFile = res.data.file.find(
                f => f.orgFileName === this.formData.custMlogoFile
              )
              this.view.uploadedFile.custWlogoFile = res.data.file.find(
                f => f.orgFileName === this.formData.custWlogoFile
              )
            }
    ...
    fnCheckDuplicateCustNm() {
      ...
      if (
        ...
        this.formData.custLoginId.length > 33
      ) {
      ...
      requestApi({
        ...
        headers: { 'X-MID': this.xMid },
        method: 'post',
        data: { custNm: this.formData.custNm, custSeq: this.data.custSeq },
        callback: res => {
          if (res.result.status === '200' || res.result.status === '204') {
            this.view.duplicate.custNm = true
