...
export default {
  ...
  methods: {
    fnSearchModifyData() {
      const paramObj = {}
      paramObj.openApiSeq = this.data.idx
      requestApi({
        url: '/online/openApi/retrieveOpenApi',
        headers: {
          'X-MID': this.xMid
        },
        method: 'post',
        ...
        callback: (res) => {
          if (res.result.status === '200' || res.result.status === '204') {
            ...
            this.paramList = JSON.parse(this.dataObj.reqParamObj)
            this.resList = JSON.parse(this.dataObj.resDataObj)
          }
        }
      })
    },
    ...
    fnSaveOpenApi() {
      const data = {
        ...
        apiDesc: this.dataObj.apiDesc
      }
        data.reqParamObj = JSON.stringify(this.paramList)
        data.resDataObj = JSON.stringify(this.resList)
        data.openApiSeq = this.data.idx

      requestApi({
        url: 'online/openApi/updateOpenApi',
        headers: {
          'X-MID': this.xMid
        },
        method: 'post',
    ...
    fnValidateData() {
      ...
      // 설명을 입력하지 않은 경우
      if (isEmpty(this.dataObj.apiDesc)) {
