<template>
  <div>
    <div
      style="z-index:99; top:0px; width:100%; height:100%; position:absolute; background-color:#000; opacity:0.2;"
      v-if="disDiv"
    ></div>
      ...
      <div class="form-row row">
          ...
          <div>
            ...
            >
              <thead>
                <tr>
                  <th class="p-0">
                    <span class="lb-txt mt-3 mb-2">{{ $t('mdlmMtCol') }}</span>
                  </th>
                </tr>
              </thead>
              <tbody v-if="mdlColList.length > 0">
                <tr
                  class="py-2"
                  v-for="(element, idx) in mdlColList"
                  v-bind:key="idx"
                >
                  <td
                    class=""
                    @click="fnMdlColSel(element.seq, element.mdlColNm)"
                  >
                    {{ element.mdlColNm }}
                  </td>
                </tr>
        ...
        <div class="col-sm-12 col-md-4 col-lg-6 pl-1 pr-1">
          ...
          <div v-if="mapInfoList.length > 0">
            <div class="px-1 py-2" style="border:1px solid #d4dce4;">
              ...
              >
                <div
                  class="row ml-0"
                  v-for="(obj, idx) in mapInfoList"
                  v-bind:key="idx"
                >
                  <div class="col-lg-5 col-sm-12 mb-2">
                    <input
                      v-model="obj.mdlColNm"
                      class="form-control"
                      disabled
                    />
                  </div>
                  <div class="col-lg-5 col-sm-12 mb-2">
                    <input
                      v-model="obj.dvcTbColNm"
                      class="form-control"
                      disabled
                    />
                  </div>
                  <div class="col-lg-2 col-sm-12 mb-2 pl-1">
                    <div class="input-group pl-2">
                      <button class="btn" type="button" @click="fnMapDel(idx)">
                        <span class="glyphicon glyphicon-minus"></span>
                      </button>
                      <div class="btn attr-list-handle">
          ...
          <div
            ...
            style="border:1px solid #d4dce4;"
          >
            <span>{{ $t('mdlMapMtColListNull') }}</span>
          </div>
          ...
          <div>
            ...
            >
              ...
              <tbody v-if="dvcColList.length > 0">
                <tr
                  class="py-2"
                  v-for="(element, idx) in dvcColList"
                  v-bind:key="idx"
                >
                  <td
                    class=""
                    @click="fnDvcColSel(element.seq, 'DEV', element.dvcColNm)"
                  >
                    {{ element.dvcColNm }}
                  </td>
                </tr>
              ...
              <tbody v-if="outTbColList.length > 0">
                <tr
                  class="py-2"
                  v-for="(element, idx) in outTbColList"
                  v-bind:key="idx"
                >
                  <td
                    class=""
                    @click="fnDvcColSel(element.seq, 'EXT', element.tbColNm)"
                  >
                    {{ element.tbColNm }}
                  </td>
                </tr>
...
export default {
  ...
  created() {
    ...
    if (
      this.data.obj.mlApiTableType === 'API' &&
      this.data.obj.mlDelayTypeCd === 'RTM'
    ) {
  ...
  methods: {
    ...
    fnMapAdd() {
      const findSameInfoItem = this.mapInfoList.findIndex(item => {
        return (
          item.mdlColNm === this.seleMdlColName &&
          item.dvcTbColNm === this.seleDevColName
        )
      })

      // 속성 선택하지 않은 경우
      if (this.seleMdlColName === '' || this.seleDevColName === '') {
        this.alert(this.$t('mdlMapMtColNulChk'), 'warning')
        // 중복된 속성이 존재할 경우
      } else if (findSameInfoItem !== -1) {
        this.alert(this.$t('mdlMapMtColDuplchkChk'), 'warning')
      } else {
        this.mapInfoList.push({
          dataType: this.seleDvcTbType,
          mdlAttbSeq: this.seleMdlColSeq,
          devAttbSeq: this.seleDevColSeq,
          mdlColNm: this.seleMdlColName,
          dvcTbColNm: this.seleDevColName
        })
      }
    },
    fnMapDel(idx) {
        ...
        callback: res => {
          if (res.result.status === '200' || res.result.status === '204') {
            ...
            for (var i = 0; i < res.data.dataList.length; i++) {
              ...
              local.mdlColList.push({
                mdlColNm: obj.mdlColCd + '(' + obj.mdlColNm + ')',
                seq: obj.seq
              })
            }
          }
      ...
      requestApi({
        ...
        callback: res => {
          if (res.result.status === '200' || res.result.status === '204') {
            ...
            for (var i = 0; i < res.data.dataList.length; i++) {
              obj = res.data.dataList[i]
              local.dvcColList.push({
                dvcColNm: obj.dvcColNm + '(' + obj.devMdlAtbNm + ')',
                seq: obj.seq
              })
            }
          }
      ...
      requestApi({
        ...
        callback: res => {
          if (res.result.status === '200' || res.result.status === '204') {
            ...
            for (var i = 0; i < res.data.dataList.length; i++) {
              obj = res.data.dataList[i]
              local.outTbColList.push({
                tbColNm: obj.columnName + '(' + obj.comments + ')',
                seq: obj.columnName
              })
            }
          }
      ...
      this.popup({
        ...
        callback: data => {
          if (data !== 'dismiss') {
            ...
            if (
              local.dataObj.mlApiTableType === 'API' &&
              local.dataObj.mlDelayTypeCd === 'RTM'
            ) {
              this.rawMapVisible = true
            } else {
              this.rawMapVisible = false
    ...
    cancelMap() {
      if (this.colRawMsgMap === true) {
        this.confirm(this.$t('cancel-map-confirm'))
          .then(() => {
            this.colRawMsgMap = true
            this.mapInfoList = []
            this.rawMap = 'Y'
    ...
    submit() {
      ...
      for (var i = 0; i < local.mapInfoList.length; i++) {
        ...
        var ord = i + 1 + '' // 순서 셋팅
        reqData.mdlAttbMap.push({
          mdlAttbSeq: obj.mdlAttbSeq,
          devAttbSeq: obj.devAttbSeq,
