...
export default {
  ...
  methods: {
      ...
      requestApi({
        ...
        callback: res => {
          if (res.result.status === '200' || res.result.status === '204') {
            ...
            if (this.dataObj.cycleDate) {
              this.$nextTick(() => {
                this.selectedCycleDate = moment(this.dataObj.cycleDate).toDate()
              })
            }
          }
        }
    ...
    chkValid() {
      if (!this.dataObj.uri || this.dataObj.uri.trim() === '') {
        this.alert(this.$t('uriValid'), 'warning')
        return
      }
      ...
      if (
        this.dataObj.head !== null &&
        this.dataObj.head !== '' &&
        this.dataObj.head.trim() === ''
      ) {
        this.alert(this.$t('solSvr-head-msg'), 'warning')
        return
      }
      if (
        this.dataObj.params !== null &&
        this.dataObj.params !== '' &&
        this.dataObj.params.trim() === ''
      ) {
        this.alert(this.$t('solSvr-params-msg'), 'warning')
        return
      }
      if (
        this.dataObj.body !== null &&
        this.dataObj.body !== '' &&
        this.dataObj.body.trim() === ''
      ) {
        this.alert(this.$t('solSvr-body-msg'), 'warning')
        return
      }
      if (
        this.dataObj.response !== null &&
        this.dataObj.response !== '' &&
        this.dataObj.response.trim() === ''
      ) {
        this.alert(this.$t('solSvr-response-msg'), 'warning')
        return
      }
