    ...
    <div class="search-box">
      <div class="form-inline">
        ...
        <div class="form-group">
          ...
          <button class="btn btn-info" @click="fnInitialize" type="button">
            {{ $t('initializing') }}
          </button>
        </div>
      </div>
...
//      유효성체크 하고있으므로 미사용하여 주석처리, 2022-07-04 김정현
//      if (!this.endDate) {
...
//      }
    ...
    fnDeleteConfirm() {
      ...
      if (!this.deletable) {
        if (
          selectedIds.filter(id => this.postList[id].regUsrId !== this.loginId)
            .length > 0
        ) {
