...
export default {
  ...
  methods: {
      ...
      requestApi({
        ...
        callback: res => {
          if (res.data.boardList) {
            res.data.boardList = getDataObjOfList(
              res.data.boardList,
              'dataObj',
              'colData'
            )
    ...
    fnDetailPopup(id, index, contents, event) {
      if (this.colModelData[index].type === 'image') {
        this.fnImagePopup(id, index)
      } else if (this.colModelData[index].type === 'map') {
        this.fnMapPopup(id, index)
      }
    },
    fnImagePopup: _.throttle(
      function(id, index, contents, event) {
        this.popup({
          component: () => import('@/views/detection/DetectionImagePopSvc'),
          title: this.$t('detectionIamge'),
          width: '800',
          height: '700',
          data: {
            colModelData: this.colModelData,
            detList: this.detList,
            id: id,
            index: index
          },
          callback: data => {}
    ...
    fnMapPopup: _.throttle(
      function(id, index, contents, event) {
        this.popup({
          component: () => import('@/views/detection/DetectionMapPopSvc'),
          title: this.$t('detectionMap'),
          width: '800',
          height: '700',
          data: {
            colModelData: this.colModelData,
