<template>
  <div>
    ...
    <div class="mb-2 text-right">
      <button type="button" class="btn btn-pink" @click="fnOpenAddPopup">
        {{ $t('registration') }}
      </button>
      <button type="button" class="btn btn-gray" @click="fnDelete">
        {{ $t('delete') }}
      </button>
    </div>
    <jqgrid
...
export default {
  ...
  methods: {
    ...
    fnInitializeSeach() {
      this.svcCd = this.svcCdList[0].svcCd
      this.devClsCd =
        this.devClsList.length > 0 ? this.devClsList[0].devClsCd : ''
      this.devMdlCd = ''
      this.devMdlList = []
      this.searchType = 'all'
      this.searchText = ''
