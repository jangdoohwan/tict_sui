...
export default {
  ...
  computed: {
    facilityName() {
      return !this.facility.facNm || this.facility.facNm === ''
        ? this.$t('facility-check')
        : this.facility.facNm
    }
  },
  watch: {},
  created() {},
  mounted() {
    this.$refs.mapWrap.style.height =
      'calc(100vh - ' + (this.$refs.mapWrap.offsetTop + 140) + 'px)'
  ...
  methods: {
    ...
    // 장비 목록 세팅
    fnSetMapDevList() {
      if (!this.facility || !this.facility.devList || !this.facility.attbList) {
        return
      }
      let devList = []
