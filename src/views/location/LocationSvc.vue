    ...
    <div class="map-wrap" style="width:100%; height:100%;min-height:800px;">
      ...
      <div class="side-menu-wrap">
        <a class="side-open"
          ...
          ><span class="haze">{{ $t('menu') }} {{ $t('open-close') }}</span></a
        >
        <div class="side-menu">
          <div class="side-menu-body">
            ...
            <div class="tab-content">
              ...
              >
                <div class="form-inline date-half">
                  ...
                  <div class="input-group">
                    <input
                      ...
                      style="width:65%;max-width:157px;"
                      @keyup.enter="searchEquList(true)"
                    />
                    <button
                      class="btn btn-info input-group-addon"
                      type="button"
                      @click="searchEquList(true)"
                    >
                      {{ $t('search') }}
                    </button>
                  </div>
                ...
                <div
                  class="grid-container wide grid-scroll full-size table-fixed-header-wrap"
                >
                  ...
                  <div class="table-wrap">
                    <table class="table table-col">
                      ...
                      <thead>
                        <tr>
                          <th scope="col">
                            <div class="fixed-th" style="width:25%">
                              {{ $t('equipment-name') }}
                            </div>
                          </th>
                          <th scope="col">
                      ...
                      <tbody>
                        <tr
                          ...
                          @click="
                            fnSetCenter(
                              item.instLat / 10000000,
                              item.instLon / 10000000
                            )
                          "
                          style="cursor:pointer;"
                        >
                          <td>{{ item.devClsCdNm }}</td>
                          <td>{{ item.devMdlNm }}</td>
                          <td>{{ item.devUname }}</td>
                          <td>{{ item.ctn }}</td>
                        </tr>
              ...
              >
                <div class="form-inline wide mb-1">
                  <div class="input-group">
                    <input
                      ...
                      v-model="searchName"
                      class="form-control"
                      :placeholder="$t('location-name-search')"
                      :title="$t('search-input')"
                      @keyup.enter="fnGetNameList()"
                    />
                    <button
                      class="btn btn-info input-group-addon"
                      type="button"
                ...
                <div
                  class="grid-container wide grid-scroll full-size table-fixed-header-wrap pt-0"
                >
                  <div class="table-wrap">
                    <table class="table table-col text-left">
                      ...
                      <tbody>
                        <tr
                          v-for="(item, index) in nameList.poi"
                          :key="`name-${index}`"
                        >
                          <td>
                            <a
                              href="javascript:void(0)"
                              @click="fnSetCenter(item.dpy, item.dpx)"
                              >{{ item.name1 }}</a
                            >
                          </td>
                        </tr>
              ...
              >
                <div class="form-inline date-half">
                  ...
                  <select
                    ...
                    class="form-control"
                    title="시/도"
                    @change="fnGetSiGunGuList()"
                    style="width:34%"
                  >
                    <option value="">{{ $t('address-sido') }}</option>
                    <option
                      v-for="(item, index) in sidoList.adm"
                      :key="`sido-${index}`"
                      :value="item.admcode"
                      >{{ item.admname }}</option
                    >
                  ...
                  <select
                    ...
                    id="sigungu"
                    class="form-control"
                    @change="fnGetDongList()"
                    style="width:33%"
                  >
                    <option value="">{{ $t('address-sigungu') }}</option>
                    <option
                      v-for="(item, index) in siGunGuList.adm"
                      :key="`gungu-${index}`"
                      :value="item.admcode"
                      >{{ item.admname }}</option
                    >
                  ...
                  >
                    ...
                    <option
                      v-for="(item, index) in dongList.adm"
                      :key="`dong-${index}`"
                      :value="item.admcode"
                      >{{ item.admname }}</option
                    >
                ...
                <div class="form-inline overflow mb-1">
                  ...
                  <div class="input-group" style="width: 73%;float: right;">
                    <input
                      id="searchAddr"
                      v-if="addressKind === 'new'"
                      type="text"
                      v-model="searchAddr"
                      class="form-control"
                      :placeholder="$t('input-roadname')"
                      title="도로명 입력"
                      @keyup.enter="fnGetAddressList()"
                      style="width:42%;margin-right:3px;"
                    />
                    <input
                      id="searchBunji"
                      type="text"
                      v-model="searchBunji"
                      class="form-control"
                      :placeholder="$t('input-bunji')"
                      title="번지 입력"
                      @keyup.enter="fnGetAddressList()"
                      style="width:25%;border-radius: 4px;"
                    />
                    <button
                      class="btn btn-info input-group-addon"
                      type="button"
                      @click="fnGetAddressList()"
                    >
                      {{ $t('search') }}
                    </button>
                  </div>
                ...
                >
                  <div class="table-wrap">
                    <table class="table table-col text-left">
                      ...
                      <tbody>
                        <tr
                          v-for="(item, index) in addressList.adm"
                          :key="`address-${index}`"
                        >
                          <td>
                            <a
                              href="javascript:void(0)"
                              @click="fnSetCenter(item.posy, item.posx)"
                            >
      ...
      <div class="map-area">
        ...
        <div class="option-wrap">
          ...
          <ul class="map-util">
            <li class="time">
              Now : <span class="nowdate">{{ nowDate }}</span>
            </li>
            <li class="time">
              Updated : <span class="lastmdate">{{ lastUpdateDate }}</span>
            </li>
        ...
        <div class="right-wrap">
          ...
          <div class="right-menu">
            ...
            <ul class="rigth-menu-inner">
              <li
                v-for="(item, index) in equipmentProperty"
                :key="`devProp-${index}`"
              >
                <div
                  class="form-check"
                  :title="item.devClsCdNm + '(' + item.cntTotal + ')'"
                  :alt="item.devClsCdNm + '(' + item.cntTotal + ')'"
                >
                  <label class="form-check-label" :for="item.devClsCd + '_all'">
                    <input
                      class="form-check-input 1depth"
                      type="checkbox"
                      :id="item.devClsCd + '_all'"
                      checked="checked"
                      @click="fnOneDepthClick"
                    />{{ item.devClsCdNm }}({{ item.cntTotal }})
                    <span class="form-check-sign"></span>
                  </label>
                </div>
                <ul>
                  <li>
                    <div class="form-check">
                      <label
                        ...
                        :for="item.devClsCd + '_true'"
                      >
                        <input
                          class="form-check-input 2depth"
                          type="checkbox"
                          :id="item.devClsCd + '_true'"
                          checked="checked"
                          @click="fnTwoDepthClick"
                        />{{ $t('normal') }}({{ item.cntTrue }})
                        <span class="form-check-sign"></span>
                      </label>
                    </div>
                  </li>
                  <li>
                    <div class="form-check">
                      <label
                        class="form-check-label"
                        :for="item.devClsCd + '_false'"
                      >
                        <input
                          class="form-check-input 2depth"
                          type="checkbox"
                          :id="item.devClsCd + '_false'"
                          checked="checked"
                          @click="fnTwoDepthClick"
                        />{{ $t('abnormal') }}({{ item.cntFalse }})
                        <span class="form-check-sign"></span>
                      </label>
                    </div>
                  </li>
                  <li>
                    <div class="form-check">
                      <label
                        class="form-check-label"
                        :for="item.devClsCd + '_stop'"
                      >
                        <input
                          class="form-check-input 2depth"
                          type="checkbox"
                          :id="item.devClsCd + '_stop'"
                          checked="checked"
                          @click="fnTwoDepthClick"
                        />{{ $t('stop') }}({{
                          item.cntStop ? item.cntStop : '0'
                        }})
                        <span class="form-check-sign"></span>
                      </label>
                    </div>
  ...
  methods: {
    ...
    fnTwoDepthClick(e) {
      let obj = $(e.target)
        ...
        .parent()
      if (
        $(obj).find('input.2depth:checked').length ===
        $(obj).find('input.2depth').length
      ) {
        $(obj)
          .find('input.1depth')
          .prop('checked', true)
    ...
    clickEventHandler(evt) {
      let element = document.getElementById('popup')
      let feature = this.map.forEachFeatureAtPixel(evt.pixel, function(
        feature
      ) {
        return feature
      })
    ...
    fnGetMultiDevieList(feature, element) {
      let context = this
      let contents =
        ' <div class="table-wrap" style="float:left;width:150px;margin-top:30px;overflow-y:auto;max-height:300px;">' +
        '    <table class="table table-col">' +
        '    <colgroup>' +
      ...
      requestApi({
        ...
        callback: res => {
          if (res.result.status === '200' || res.result.status === '204') {
            let contents =
              '<div class="map-popover" style="float:right;width:360px;">' +
              ...
              '    <table class="table table-col">' +
              '    <caption>' +
              this.$t('equipment-attribute-search') +
              '</caption>' +
              '    <colgroup>' +
              ...
              '    <tr>' +
              '      <th scope="col">' +
              this.$t('equipment-attribute') +
              '</th>' +
              '      <th scope="col">' +
              this.$t('equipment-content') +
              '</th>' +
              '    </tr>' +
              '    </thead>' +
              '    <tbody>'
            // CTN/상태/장비유형/식별명/설치장소 는 기본 장비 목록 정보에서 가져와서 처리한다.
            let defaultContent = this.equipmentListTemp.filter(
              list => list.entrDevSeq === entryDevSeq
            )
            ...
            contents +=
              ...
              ') </td>' +
              '      <td>' +
              defaultContent[0].ctn +
              '</td>' +
              '    </tr>' +
              '    <tr>' +
              '      <td>' +
              this.$t('equipment-status') +
              '</td>' +
              '      <td>' +
              this.fnGetStatusDesc(defaultContent[0].attbStatusCd) +
              '</td>' +
              '    </tr>' +
              '    <tr>' +
              '      <td>' +
              this.$t('equipment-name') +
              '</td>' +
              '      <td>' +
              defaultContent[0].devClsCdNm +
              '</td>' +
              '    </tr>' +
              '    <tr>' +
              '      <td>' +
            ...
            res.data.forEach(data => {
              contents += '    <tr>' + '      <td>' + data.devAttbCdNm + '</td>'
            ...
            $(element).popover({
              ...
              html: true,
              content: bln
                ? this.featureListHTML + this.featureDetailHTML
                : this.featureDetailHTML
            })
            $(element).popover('show')
            this.touchScroll()
            if (bln) {
          ...
          if (res.result.status === '200' || res.result.status === '204') {
            ...
            res.data.list.forEach((data, index) => {
              let coordinates = proj.transform(
                [data.instLon / 10000000, data.instLat / 10000000],
                this.WGS84,
                this.MERCATOR
              )
              let statusCd =
                data.attbStatusCd === 'A'
                  ...
                  ? 'false'
                  : 'stop'
              let deviceView = res.data.view.find(
                view => view.devClsCd === data.devClsCd
              )

              // 장비목록에는 있지만 장비속성탭에는 없는 모델인 경우 기본 값으로 장비속성 데이터에 추가
              if (!deviceView) {
                let obj = res.data.list.filter(
                  list => list.devClsCd === data.devClsCd
                )
                let objA = res.data.list.filter(
                  list =>
                    list.devClsCd === data.devClsCd && data.attbStatusCd === 'A'
                )
                let objW = res.data.list.filter(
                  list =>
                    list.devClsCd === data.devClsCd && data.attbStatusCd === 'W'
                )
                let objS = res.data.list.filter(
                  list =>
                    list.devClsCd === data.devClsCd && data.attbStatusCd === 'S'
                )
                ...
                deviceView = {
                  ...
                  norGroup: 'assets/G_TB_A.png',
                  warGroup: 'assets/G_TB_W.png',
                  stopGroup: 'assets/G_TB_S.png'
                }
              ...
              if (data.attbStatusCd === 'A') {
                if (!deviceView.norGroup) {
                  deviceView.norGroup = 'assets/G_TB_A.png'
                }
              } else if (data.attbStatusCd === 'W') {
                if (!deviceView.warGroup) {
                  deviceView.warGroup = 'assets/G_TB_W.png'
                }
              } else {
                if (!deviceView.stopGroup) {
                  deviceView.stopGroup = 'assets/G_TB_S.png'
                }
              }
              ...
              this.featuresOri.push(
                new OlFeature({
                  ...
                  icon: data.imgFile
                    ? data.imgFile
                    : 'assets/TB_' + data.attbStatusCd + '.png',
                  groupIcon:
                    data.attbStatusCd === 'A'
                      ? deviceView.norGroup
                      : data.attbStatusCd === 'W'
                      ? deviceView.warGroup
                      : deviceView.stopGroup,
    ...
    clusterLayerStyle(feature) {
      ...
      if (size === 1 || this.getZoomLevel() >= 16) {
        ...
        if (size === 1) {
          ...
          // 장비가 2개 이상인 경우
          clusterImg = new OlStyleIcon({
            src: '/sui/external/map/images/multi_gas.png'
          })
          ...
          if (stopCnt + warningCnt > 0) {
            clusterImg = new OlStyleIcon({
              src: '/sui/external/map/images/multi_gas_critial.png'
            })
          }
        }
      } else {
        ...
        if (flagCnt > 0) {
          clusterImg = new OlStyleIcon({ src: 'assets/G_TB_W.png' })
        } else {
          clusterImg = new OlStyleIcon({ src: 'assets/G_TB_A.png' })
        }
      }
    ...
    fnGetAddressList() {
      ...
      reqData.api =
        this.addressKind === 'new' ? 'searchNewAddr' : 'searchOldAddr'
      reqData.headers = headers
      reqData.body = body
      ...
      requestApi({
        ...
        callback: res => {
          if (res.result.status === '200' || res.result.status === '204') {
            ...
            if (
              this.addressKind === 'new' &&
              parseInt(this.addressList.count, 10) > 0
            ) {
    ...
    fnSetCenter(instLat, instLon) {
      ...
      this.mapView.setCenter(
        proj.transform(
          [instLon / 10000000, instLat / 10000000],
          this.WGS84,
          this.MERCATOR
        )
      )
