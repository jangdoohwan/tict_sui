        ...
        <ul class="form-group">
          ...
          >
            ...
            <span style="width:60%;float:left;;padding: 5px 10px;">
              ...
              <span
                style="width:30%;float:left;padding-top:10px;font-size:10px;"
                >{{ attbs.unit }}</span
              >
            </span>
          </li>
...
export default {
  ...
  methods: {
    ...
    fnUsingNoDuplChk() {
      if (this.usingNo === null || this.usingNo.trim() === '') {
    ...
    fnUuidDuplChk() {
      if (this.devUuid === null || this.devUuid.trim() === '') {
    ...
    fnUnameDuplChk() {
      if (this.devUname === null || this.devUname.trim() === '') {
    ...
    fnInstNoDuplChk() {
      if (this.instNo === null || this.instNo.trim() === '') {
    ...
    createEntrDev() {
      ...
      reqData.iagStatusCd = '-'
      reqData.rtspUri = this.rtspUri
      reqData.accuUri = this.accuUri
      var entrDevAttbs = []
    ...
    updateEntrDev() {
      ...
      updateReqData.rtspUri = this.rtspUri
    ...
    checkValid() {
      // 식별번호(CTN) 빈 값
      if (this.ctn === null || this.ctn.trim() === '') {
        this.alert(this.$t('edev-ctn-chk-msg'))
        return false
      }
      // 식별번호(CTN) 중복 확인
      ...
      // 설치주소 빈 값
      if (this.instAddr === null || this.instAddr.trim() === '') {
        this.alert(this.$t('edev-instAddr-chk-msg'))
        return false
      }
      // 상세주소 빈 값
      if (this.instAddrDetail === null || this.instAddrDetail.trim() === '') {
      ...
      // 시설명 공백
      if (
        this.devBuildingNm !== null &&
        this.devBuildingNm !== '' &&
        this.devBuildingNm.trim() === ''
      ) {
      ...
      // 시설구분 공백
      if (
        this.devBuildingType !== null &&
        this.devBuildingType !== '' &&
        this.devBuildingType.trim() === ''
      ) {
      ...
      // rtsp 주소
      // if (
      //   (this.rtspUri !== '' && this.rtspUri !== null) &&
      //   !this.rtspUri.startsWith('rtsp://')
      // ) {
      ...
      // 제어 주소
      if (
        (this.accuUri !== '' && this.accuUri !== null) &&
        !(this.accuUri.startsWith('http://') || this.accuUri.startsWith('https://'))
      ) {
    ...
    fnSearchAddr() {
      // 설치주소 빈 값 확인
      if (!this.instAddr || this.instAddr.trim() === '') {
        this.alert(this.$t('edev-instAddr-chk-msg'))
      } else {
        let reqData = {}
