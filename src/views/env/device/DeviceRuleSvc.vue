<template>
  <div>
    ...
    <div class="search-box lb-txt-in">
      <div class="left">
        <div class="form-inline">
          ...
          <div class="form-group">
            <span class="form-label mx-1">{{ $t('terminal-type') }}</span>
            <select v-model="devClsCd" class="form-control">
              <option
                v-for="(devCls, idx) in devClsList"
                :key="'devCls_' + idx"
    ...
    <div class="grid-container grid-scroll">
            ...
            <table class="table table-col" style="min-width: 600px;">
              ...
              <thead>
                <tr>
                  <th
                    scope="col"
                    v-for="(colNm, idx) in modelConversionTable.colNameData"
                    :key="'modelConversionTableCol_' + idx"
                  >
                    {{ colNm }}
                  </th>
                </tr>
              ...
              <tbody>
                <tr
                  v-for="(row, idx) in modelConversionTable.data"
                  :key="'modelConversionRow_' + idx"
                  @dblclick="
                    selectModelTable(
                      row.devSeq,
                      row.devMdlCd,
                  ...
                  "
                  :class="{
                    bg_color: row.devSeq === modelConversionTable.selectDevSeq
                  }"
                >
                  <td class="left">
                    <div>{{ row.devClsCdNm }}</div>
                  </td>
            ...
            <table class="table table-col half" style="min-width: 600px;">
              ...
              <thead>
                <tr>
                  <th
                    scope="col"
                    v-for="(colNm, idx) in devConversionTable.colNameData"
                    :key="'devConversionTableCol_' + idx"
                  >
                    {{ colNm }}
                  </th>
                </tr>
              ...
              <tbody>
                <tr
                  v-for="(row, idx) in devConversionTable.data"
                  :key="'devConversionRow_' + idx"
                  @dblclick="
                    selectApiRuleTable(row.apiSeq, row.apiNm, row.serverGubun)
                  "
                  :class="{ bg_color: row.apiSeq === apiTable.selectApiSeq }"
                >
                  <td>{{ row.ruleUri }}</td>
            ...
            <table class="table table-col" style="min-width:1000px;">
              ...
              <thead>
                <tr>
                  <th
                    scope="col"
                    v-for="(colNm, idx) in apiTable.colNameData"
                    :key="'apiTableCol_' + idx"
                  >
                    {{ colNm }}
                  </th>
                </tr>
              ...
              <tbody>
                <tr
                  v-for="(row, idx) in apiTable.data"
                  :key="'apiRow_' + idx"
                  @dblclick="selectApiTable(row.apiSeq, row.apiNm)"
                  :class="{ bg_color: row.apiSeq === apiTable.selectApiSeq }"
                >
      ...
      <div
        v-show="retrieveData.divisionCd === 'SD00000002'"
        class="row"
          ...
          <div class="left">
            <div class="form-inline">
              <div class="form-group overflow">
                <span class="form-control-static float-left">{{
                  $t('server-nm')
                }}</span>
                <select
                  v-model="serverNm"
                  name=""
                  class="form-control float-left input-mt select-h-29"
                  @change="changeTabList('S')"
                >
                  <option
                    v-for="(server, idx) in serverNmList"
                    :key="'server_' + idx"
                    :value="server.cdId"
                    >{{ server.cdNm }}</option
                  >
              ...
              <div class="form-group overflow">
                ...
                <select
                  v-model="parsingRuleAllTab.type"
                  id="typeSelect"
                  name="typeSelect"
                  class="form-control float-left input-mt select-h-29"
                  ...
                  :disabled="
                    serverNm === 'GN00000047' &&
                      tabInfo.selectedTab !== 'tab1' &&
                      tabInfo.selectedTab !== 'tab2'
                  "
                >
                  <option
                    v-for="(type, idx) in parsingRuleAllTab.typeList"
                    :key="'type_' + idx"
                    :value="type.cdId"
                    >{{ type.cdNm }}</option
                  >
              ...
              <div class="form-group overflow">
                ...
                <input
                  v-model="devApiNm"
                  type="text"
                  class="form-control float-left input-mt"
                  placeholder=""
                  title="검색어 입력"
                  disabled
                  @change="changeTabList('A')"
                />
              </div>
              <div class="form-group">
                <button
              ...
              <div class="form-group overflow" v-if="chkServerType">
                ...
                >
                  ...
                  <option
                    v-for="(server, idx) in svrMsgTypeList"
                    :key="'svrMsgType_' + idx"
                    :value="server.cdId"
                    >{{ server.cdNm }}</option
                  >
      ...
      <!-- 수집 / 제어 정보 설정 -->
      <div class="row col-md-12" style="width: 102%;">
        <div class="tit-wrap">
          <h4 class="titH4" v-if="this.retrieveData.divisionCd !== ''">
            수집 / 제어 정보설정 {{ tabText }}
          </h4>
        ...
        <div role="tabpanel" class="rule-wrap">
          <ul class="nav nav-tabs" role="tablist">
            <li
              v-show="parsingRuleAllTabVisible"
              :class="getTabNameLiClass('tab1')"
              @click="selectTab('tab1')"
              role="presentation"
            >
              <a aria-controls="tab1" role="tab" data-toggle="tab"
                >파싱룰(전문)</a
              >
            </li>
            <li
              v-show="pasingRuleTabVisible"
              :class="getTabNameLiClass('tab2')"
              @click="selectTab('tab2')"
              role="presentation"
            >
              <a aria-controls="tab2" role="tab" data-toggle="tab">파싱룰</a>
            </li>
            <li
              v-show="verificationRuleTabVisible"
              :class="getTabNameLiClass('tab3')"
              @click="selectTab('tab3')"
              role="presentation"
            >
              <a aria-controls="tab3" role="tab" data-toggle="tab">검증룰</a>
            </li>
            <li
              v-show="mappingRuleTabVisible"
              :class="getTabNameLiClass('tab4')"
              @click="selectTab('tab4')"
              role="presentation"
            >
              <a aria-controls="tab4" role="tab" data-toggle="tab">매핑룰</a>
            </li>
            <li
              v-show="mappingCtlTabVisible"
              :class="getTabNameLiClass('tab6')"
              @click="selectTab('tab6')"
              role="presentation"
            >
              <a aria-controls="tab6" role="tab" data-toggle="tab">제어룰</a>
            </li>
          </ul>
          <!-- Tab panes -->
          <div class="tab-content">
            <!-- Tab 01 파싱룰(전문) -->
            <div
              v-show="isSelectedTab('tab1')"
              role="tabpanel"
              class="tab_margin_style"
            >
              <div class="row tab_margin_style">
                <div class="col-md-12">
                  <div class="form-inline">
                    ...
                    <span
                      v-show="parsingRuleAllTab.isSaveParseHeadList"
                      class="lb-txt w100"
                      >설정됨</span
                    >
                    <span
                    ...
                    <table class="table table-col" style="min-width:1600px;">
                      ...
                      <thead>
                        <tr>
                          <th
                            v-for="(colNm,
                            idx) in parsingRuleAllTab.colNameData"
                            :key="'parsingRuleAllTableNm_' + idx"
                          >
                            {{ colNm }}
                          </th>
                          <th></th>
                      ...
                      <tbody>
                        <tr
                          v-for="(row,
                          idx) in parsingRuleAllTab.parsingRuleAllHeadList"
                          :key="'parsingHead_' + idx"
                        >
                          <td>
                            <select
                              class="form-control"
                              v-model="row.ruleRequest"
                              title="전문KEY"
                              :disabled="row.exeFile === '6'"
                            >
                              <option
                                v-for="(ruleRequest,
                                idx) in tabDataList.proKeyList"
                                :key="'ruleRequest_' + idx"
                                :value="ruleRequest.cdId"
                                >{{ ruleRequest.cdNm }}</option
                              >
                            </select>
                          </td>
                          <td>
                          ...
                          <td>
                            <select
                              class="form-control"
                              v-model="row.ruleTargetkey"
                              title="목적키"
                              @change="changeHeaderTagetKey(idx)"
                            >
                              <option
                                v-for="(ruleTargetkey,
                                idx) in parsingRuleAllTab.headPurposeKeyList"
                                :key="'ruleTargetkey_' + idx"
                                :value="ruleTargetkey.cdId"
                                >{{ ruleTargetkey.cdNm }}</option
                              >
                            </select>
                          </td>
                          <td>
                            <select
                              ...
                              @change="
                                changeExeFile(
                                  parsingRuleAllTab.parsingRuleAllHeadList,
                                  idx,
                                  row.exeFile
                                )
                              "
                            >
                              <option
                                v-for="(exe, idx) in tabDataList.exeFileList"
                                :key="'exeFile_' + idx"
                                :value="exe.cdId"
                                >{{ exe.cdNm }}</option
                              >
                          ...
                          <td>
                            <select
                              class="form-control"
                              v-model="row.ruleNumber"
                              title="수"
                              :disabled="
                                row.exeFile === '6' || row.exeFile === '9'
                              "
                            >
                              <option
                                v-for="(ruleNumber,
                                idx) in tabDataList.numberList"
                                :key="'ruleNumber_' + idx"
                                :value="ruleNumber.cdId"
                                >{{ ruleNumber.cdNm }}</option
                              >
                            </select>
                          </td>
                          <td>
                            ...
                            >
                              <option
                                v-for="(rulePoint,
                                idx) in tabDataList.decimalPointList"
                                :key="'rulePoint_' + idx"
                                :value="rulePoint.cdId"
                                >{{ rulePoint.cdNm }}</option
                              >
                            </select>
                          </td>
                          <td>
                            <div class="py-1 text_left">
                              <button
                                ...
                                class="btn"
                                type="button"
                                @click="insertRowParsingRuleAllHeadTable"
                              >
                              ...
                              >
                                ...
                                <button
                                  class="btn"
                                  type="button"
                                  @click="deleteRowParsingRuleAllHeadTable(idx)"
                                >
                                  <span
              ...
              <div class="mt-2 text-right">
                <button
                  class="btn btn-info"
                  type="button"
                  :disabled="disabledSaveButtonControl"
                  @click="fnCallInsertApi('parsingRuleAllHead')"
                >
                  {{ $t('save') }}
                </button>
              </div>
              <div class="row tab_margin_style">
                <div class="col-md-12">
                  <div
                    v-show="retrieveData.devApiNm === 'TCPCollector'"
                    class="form-inline"
                  >
                    <!-- 타입 세팅 -->
                    <div class="form-group overflow">
                      <span
                        v-show="parsingRuleAllTab.isSaveParseBodyList"
                        class="lb-txt w100"
                  ...
                  <div class="table-wrap auto overflow_x_scroll">
                    <table class="table table-col" style="min-width: 1500px;">
                      ...
                      <tbody>
                        <tr
                          v-for="(row,
                          idx) in parsingRuleAllTab.parsingRuleAllBodyList"
                          :key="'parsingBody_' + idx"
                        >
                          <td>
                          ...
                          <td>
                            ...
                            >
                              <option
                                v-for="(ruleTargetkey,
                                idx) in tabDataList.purposeKeyList"
                                :key="'ruleTargetkey_' + idx"
                                :value="ruleTargetkey.cdId"
                                >{{ ruleTargetkey.cdNm }}</option
                              >
                          ...
                          <td>
                            <select
                              ...
                              title="실행파일"
                              @change="
                                changeExeFileParsingRuleAllBody(
                                  parsingRuleAllTab.parsingRuleAllBodyList,
                                  idx,
                                  row.exeFile
                                )
                              "
                            ...
                            <div class="py-1 text_left">
                              ...
                              >
                                <button
                                  class="btn"
                                  type="button"
                                  @click="insertRowParsingRuleAllBodyTable"
                                >
                                  <span class="glyphicon glyphicon-plus"></span>
                                </button>
                                <button
                                  class="btn"
                                  type="button"
                                  @click="deleteRowParsingRuleAllBodyTable(idx)"
                                >
                                  <span
                                    class="glyphicon glyphicon-minus"
                                  ></span>
              ...
              <div class="mt-2 text-right">
                <button
                  ...
                  type="button"
                  :disabled="disabledSaveButtonControl"
                  @click="fnCallInsertApi('parsingRuleAllBody')"
                >
                  {{ $t('save') }}
                </button>
              </div>
            ...
            >
              <div class="row tab_margin_style">
                <div class="col-md-12">
                  <div class="form-inline">
                    ...
                    <span
                      v-show="parsingRuleTab.isSaveParseList"
                      class="lb-txt w100"
                      >설정됨</span
                    >
                    <span
                  ...
                  <div class="table-wrap overflow_x_scroll">
                    <table class="table table-col" style="min-width: 1600px;">
                      ...
                      <thead>
                        <tr>
                          <th
                            v-for="(colNm, idx) in parsingRuleTab.colNameData"
                            :key="'parsingRuleTableNm_' + idx"
                          >
                            {{ colNm }}
                          </th>
                        </tr>
                      ...
                      <tbody>
                        <tr
                          v-for="(row, idx) in parsingRuleTab.parsingRuleList"
                          :key="'parsing_' + idx"
                        >
                          <td>
                            <!-- <select class="form-control" v-model="row.ruleRequest" title="전문KEY" :disabled="row.exeFile === '6'"> 임시조치-->
                          ...
                          <td>
                            <select
                              ...
                              @change="
                                changeExeFile(
                                  parsingRuleTab.parsingRuleList,
                                  idx,
                                  row.exeFile
                                )
                              "
                          ...
                          <td>
                            <div class="py-1 text_left">
                              <div
                                v-if="
                                  idx === 0 &&
                                    parsingRuleTab.parsingRuleList.length === 1
                                "
                              >
                                <button
                                  class="btn"
                                  type="button"
                                  @click="insertRowParsingRuleTable"
                                >
                                  <span class="glyphicon glyphicon-plus"></span>
                                </button>
                                <button
                                  class="btn"
                                  type="button"
                                  @click="deleteRowParsingRuleTable(idx)"
                                >
                                  <span
                                    class="glyphicon glyphicon-minus"
                                  ></span>
              ...
              <div class="mt-2 text-right">
                <button
                  ...
                  type="button"
                  :disabled="disabledSaveButtonControl"
                  @click="fnCallInsertApi('parsingRule')"
                >
                  {{ $t('save') }}
                </button>
              </div>
            ...
            >
              <div class="row tab_margin_style">
                <div class="col-md-12">
                  <span
                    v-show="
                      retrieveData.devApiNm !== 'TCPCollector' &&
                        verificationRuleTab.isSaveValList
                    "
                    class="lb-txt w100"
                    >설정됨</span
                  >
                  ...
                  <div
                    v-show="
                      retrieveData.devApiNm === 'TCPCollector' &&
                        retrieveData.divisionCd === 'SD00000002'
                    "
                    class="form-inline"
                  >
                    <!-- 타입 세팅 -->
                    <div class="form-group overflow">
                      ...
                      <span
                        v-show="verificationRuleTab.isSaveValList"
                        class="lb-txt w100"
                        style="margin: 7px 0 0 7px"
                        >설정됨</span
                      >
                  ...
                  <div class="table-wrap auto overflow_x_scroll">
                    <table class="table table-col" style="min-width: 1650px;">
                      ...
                      <colgroup>
                        ...
                        <col style="width:100px;" />
                        <col
                          v-if="retrieveData.divisionCd === 'SD00000002'"
                          style="width:120px;"
                        />
                      ...
                      <thead>
                        <tr>
                          <th
                            v-for="(colNm,
                            idx) in verificationRuleTab.colNameData"
                            :key="'parsingRuleTableNm_' + idx"
                          >
                            {{ colNm }}
                          </th>
                      ...
                      <tbody>
                        <tr
                          v-for="(row,
                          idx) in verificationRuleTab.verificationRuleList"
                          :key="'verificationRuleRow_' + idx"
                        >
                          <td v-if="retrieveData.divisionCd === 'SD00000002'">
                            ...
                            >
                              <option
                                v-for="(ruleRequest,
                                idx) in verificationRuleTab.proKeyList"
                                :key="'ruleRequest_' + idx"
                                :value="ruleRequest.cdId"
                                >{{ ruleRequest.cdNm }}</option
                              >
                          ...
                          <td v-if="retrieveData.divisionCd === 'SD00000002'">
                            <select
                              class="form-control"
                              v-model="row.valRuleName"
                              title="KEY"
                              disabled
                            >
                              <option
                                v-for="(key,
                                idx) in verificationRuleTab.valKeyList"
                                :key="'key_' + idx"
                                :value="key.cdId"
                                >{{ key.cdNm }}</option
                              >
                          ...
                          <td>
                            <select
                              class="form-control"
                              v-model="row.defaultYn"
                              title="필수여부"
                            >
                              <option
                                v-for="(yn, idx) in tabDataList.ynList"
                                :key="'defaultYn_' + idx"
                                :value="yn.cdId"
                                >{{ yn.cdNm }}</option
                              >
                          ...
                          <td v-if="retrieveData.divisionCd === 'SD00000002'">
                            <div class="py-1 text_left">
                              <div
                                v-if="
                                  idx === 0 &&
                                    verificationRuleTab.verificationRuleList
                                      .length === 1
                                "
                              >
                                <button
                                  class="btn"
                                  type="button"
                                  @click="insertRowVerificationRuleTable"
                                >
                                  <span class="glyphicon glyphicon-plus"></span>
                                </button>
                                <button
                                  class="btn"
                                  type="button"
                                  @click="deleteRowVerificationRuleTable(idx)"
                                >
                                  <span
                                    class="glyphicon glyphicon-minus"
                                  ></span>
              ...
              <div class="mt-2 text-right">
                <button
                  class="btn btn-info"
                  type="button"
                  :disabled="!this.apiTable.selectApiSeq"
                  @click="fnCallInsertApi('verificationRule')"
                >
                  {{ $t('save') }}
                </button>
              </div>
            ...
            >
              <div class="row tab_margin_style">
                <div class="col-md-12">
                  ...
                  >
                    <span
                      v-show="mappingRuleTab.isSaveMapList"
                      class="lb-txt w100"
                      >설정됨</span
                    >
                    <span
                  ...
                  <div class="table-wrap auto overflow_x_scroll">
                    <table class="table table-col" style="min-width: 1480px;">
                      ...
                      <thead>
                        <tr>
                          <th
                            v-for="(colNm, idx) in mappingRuleTab.colNameData"
                            :key="'mappingRuleTableNm_' + idx"
                          >
                            {{ colNm }}
                          </th>
                        </tr>
                      ...
                      <tbody>
                        <tr
                          v-for="(row, idx) in mappingRuleTab.mappingRuleList"
                          :key="'mapping_' + idx"
                        >
                          <td>
                            <select
                          ...
                          <td>
                            ...
                            >
                              <option
                                v-for="(key,
                                idx) in mappingRuleTab.purposeKeyList"
                                :key="'key_' + idx"
                                :value="key.cdId"
                                >{{ key.cdNm }}</option
                              >
                          ...
                          <td>
                            <select
                              ...
                              title="실행파일"
                              @change="
                                changeExeFileMappingRule(
                                  mappingRuleTab.mappingRuleList,
                                  idx,
                                  row.exeFile
                                )
                              "
                          ...
                          <td>
                            <div class="py-1 text_left">
                              <div
                                v-if="
                                  idx === 0 &&
                                    mappingRuleTab.mappingRuleList.length === 1
                                "
                              >
                                <button
                                  class="btn"
                                  type="button"
                                  @click="insertRowMappingRuleTable"
                                >
                                  <span class="glyphicon glyphicon-plus"></span>
                                </button>
                                <button
                                  class="btn"
                                  type="button"
                                  @click="deleteRowMappingRuleTable(idx)"
                                >
                                  <span
                                    class="glyphicon glyphicon-minus"
                                  ></span>
              ...
              <div class="mt-2 text-right">
                <button
                  ...
                  type="button"
                  :disabled="disabledSaveButtonControl"
                  @click="fnCallInsertApi('mappingRule')"
                >
                  {{ $t('save') }}
                </button>
              </div>
            ...
            <div v-show="isSelectedTab('tab6')" role="tabpanel">
              <div class="row tab_margin_style">
                <div class="col-md-12">
                  <div class="form-inline">
                    <span
                      v-show="parsingRuleCtlTab.isSaveParseList"
                      class="lb-txt w100"
                      >설정됨</span
                    >
                    <span
                    ...
                    <table class="table table-col" style="min-width:1600px;">
                      ...
                      <thead>
                        <tr>
                          <th
                            v-for="(colNm,
                            idx) in parsingRuleCtlTab.colNameData"
                            :key="'parsingRuleCtlTab_' + idx"
                          >
                            {{ colNm }}
                          </th>
                          <th></th>
                      ...
                      <tbody>
                        <tr
                          v-for="(row,
                          idx) in parsingRuleCtlTab.parsingRuleCtlList"
                          :key="'parsingHead_' + idx"
                        >
                          <td>
                            <select
                          ...
                          <td>
                            <div class="py-1 text_left">
                              <button
                                v-if="
                                  idx === 0 &&
                                    parsingRuleCtlTab.parsingRuleCtlList
                                      .length === 1
                                "
                                class="btn"
                                type="button"
                                @click="insertRowCtlTable"
                              >
                                <span class="glyphicon glyphicon-plus"></span>
                              </button>
                              <div
              ...
              <div class="mt-2 text-right">
                <button
                  ...
                  type="button"
                  :disabled="disabledSaveButtonControl"
                  @click="fnCallInsertApi('controlRule')"
                >
                  {{ $t('save') }}
                </button>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      parsingRuleAllTab: {
        ruleColTypeHead: '',
        colNameData: [
          this.$t('commuication-format-key'),
          this.$t('key-en'),
          this.$t('purpose-key'),
          this.$t('exe-file'),
          this.$t('byte-length'),
  ...
  mounted() {
    ...
    this.fnSearchCommon(
      this.fnAddValProKeyList,
      '/online/rule/retrieveIotRequestKeyParseList',
      { parentCdId: 'GN00000173' }
    ) // 검증룰 전문KEY 리스트 조회
    this.fnSearchCommon(
      this.fnAddProKeyList,
      '/online/rule/retrieveIotRequestKeyValList',
      { parentCdId: 'GN00000173' }
    ) // 그 외 전문KEY 리스트 조회

    this.fnSearchCommon(
      this.fnAddMappingRuleTabPurposeKeyList,
      '/online/rule/retrieveIotTargetKeyList',
      {}
    ) // 매핑룰 목적키 리스트 조회
    // this.fnSearchCommon(this.fnAddPurposeKeyList, '/online/rule/retrieveIotTargetKeyParseList', {}) // 그외 목적키 리스트 조회

    this.fnSearchCommon(
      this.fnAddNumberList,
      '/online/iotcmcd/retrieveIotByParentCmCdRuntime',
      { parentCdId: 'GN00000170' }
    ) // 수 리스트 조회
    this.fnSearchCommon(
      this.fnAddDecimalPointList,
      '/online/iotcmcd/retrieveIotByParentCmCdRuntime',
      { parentCdId: 'GN00000171' }
    ) // 소수점 리스트 조회
    this.fnSearchCommon(this.fnAddExeFileList, '/online/rule/retrieveIotRule', {
      runGubun: '1'
    }) // 실행파일 리스트 조회
  },
  computed: {
    parsingRuleAllTabVisible() {
      return (
        this.retrieveData.divisionCd === 'SD00000002' &&
        this.retrieveData.serverNm === 'GN00000047' &&
        this.retrieveData.devApiNm === 'TCPCollector' &&
        this.serverNm === 'GN00000047'
      )
    },
  ...
  watch: {
    ...
    'modelConversionTable.selectDevSeq'(newVal) {
      console.log(
        '[WATCH HOOK]  modelConversionTable.selectDevSeq  /  ' + newVal
      )
      // 테이블 리스트 조기화
      this.initedTabTableList()
      // 값이 있을 때
  ...
  methods: {
    ...
    fnAddModelList(data) {
      ...
      if (data.boardList && data.boardList.length > 0) {
        this.selectModelTable(
          this.modelConversionTable.data[0].devSeq,
          this.modelConversionTable.data[0].devMdlCd,
          this.modelConversionTable.data[0].devClsCdNm,
          this.modelConversionTable.data[0].devMdlNm
        )
      } else {
    ...
    fnAddDevRuleList(data) {
      ...
      if (data.dataList && data.dataList.length > 0) {
        ...
        this.selectApiRuleTable(
          data.dataList[0].apiSeq,
          data.dataList[0].apiNm,
          data.dataList[0].serverGubun
        )
      } else {
        this.tabText =
          '(' +
    ...
    retrieveParsingRuleAllHeadList() {
      this.fnSearchCommon(
        this.fnAddParsingRuleAllHeadList,
        '/online/rule/retrieveTbIotColValRuleInfo',
        {
          ruleColType: 'GN00000183',
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          ruleMsgType: 'H' + this.parsingRuleAllTab.type
        }
      ) // 파싱룰(전문) 헤더 리스트 조회
    },
    // 파싱룰(전문) Body 리스트 조회
    retrieveParsingRuleAllBodyList() {
      this.fnSearchCommon(
        this.fnAddParsingRuleAllBodyList,
        '/online/rule/retrieveTbIotColTcpValRuleInfo',
        {
          ruleColType: 'GN00000183',
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          devMdlCd: this.modelConversionTable.selectDevMdlCd,
          ruleMsgType: this.parsingRuleAllTab.type
        }
      ) // 파싱룰(전문) body 리스트 조회
    },
    // 파싱룰 리스트 조회
    retrieveParsingRuleList() {
      this.fnSearchCommon(
        this.fnAddParsingRuleList,
        '/online/rule/retrieveTbIotColValRuleInfo',
        {
          ruleColType: 'GN00000184',
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          ruleMsgType: this.parsingRuleAllTab.type
        }
    ...
    retrieveMappingRuleList() {
      this.fnSearchCommon(
        this.fnAddMappingRuleList,
        '/online/rule/retrieveTbIotColValRuleInfo',
        {
          ruleColType: 'GN00000186',
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          ruleMsgType: this.parsingRuleAllTab.type
        }
    ...
    retrieveVerificationRuleList() {
      this.fnSearchCommon(
        this.fnAddVerificationRuleList,
        '/online/rule/retrieveTbIotValRuleInfoList',
        {
          devSeq: this.modelConversionTable.selectDevSeq,
          apiSeq: this.apiTable.selectApiSeq,
          ruleMsgType: this.parsingRuleAllTab.type
    ...
    retrieveServiceCallList() {
      this.fnSearchCommon(
        this.fnAddServiceCallList,
        '/online/rule/retrieveTbIotDevApiRule',
        {
          ruleColType: this.serviceCallTab.ruleColType,
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq
        }
      ) // 서비스 호출 리스트 조회
    ...
    retrieveCtlRuleList() {
      this.fnSearchCommon(
        this.fnAddCtlRuleList,
        '/online/rule/retrieveTbIotCtlValRuleInfo',
        {
          ruleColType: 'GN00000209',
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          ruleMsgType: this.parsingRuleAllTab.type
        }
    ...
    insertRowParsingRuleAllHeadTable() {
      this.parsingRuleAllTab.parsingRuleAllHeadList = _.cloneDeep(
        this.parsingRuleAllTab.parsingRuleAllHeadList.concat(
          this.parsingRuleAllTab.headDefaultData
        )
      )
    },
    // 파싱룰(전문) body 테이블 row 추가
    insertRowParsingRuleAllBodyTable() {
      this.parsingRuleAllTab.parsingRuleAllBodyList = _.cloneDeep(
        this.parsingRuleAllTab.parsingRuleAllBodyList.concat(
          this.parsingRuleAllTab.bodyDefaultData
        )
      )
    },
    ...
    insertRowParsingRuleTable() {
      this.parsingRuleTab.parsingRuleList = _.cloneDeep(
        this.parsingRuleTab.parsingRuleList.concat(
          this.parsingRuleTab.defaultData
        )
      )
    },
    // 검증룰 테이블 row 추가
    insertRowVerificationRuleTable() {
      this.verificationRuleTab.verificationRuleList = _.cloneDeep(
        this.verificationRuleTab.verificationRuleList.concat(
          this.verificationRuleTab.defaultData
        )
      )
    },
    // 매핑룰 테이블 row 추가
    insertRowMappingRuleTable() {
      this.mappingRuleTab.mappingRuleList = _.cloneDeep(
        this.mappingRuleTab.mappingRuleList.concat(
          this.mappingRuleTab.defaultData
        )
      )
    },
    // 서비스 호출 테이블 row 추가
    insertRowServiceCallTable() {
      this.serviceCallTab.serviceCallDataList = _.cloneDeep(
        this.serviceCallTab.serviceCallDataList.concat(
          this.serviceCallTab.defaultData
        )
      )
    },
    // 제어룰 테이블 row 추가
    insertRowCtlTable() {
      this.parsingRuleCtlTab.parsingRuleCtlList = _.cloneDeep(
        this.parsingRuleCtlTab.parsingRuleCtlList.concat(
          this.parsingRuleCtlTab.defaultData
        )
      )
    },
    // 제어룰 테이블 row 삭제
    ...
    saveParsingRuleAllHeadTable() {
      ...
      this.fnSearchCommon(
        this.saveParsingRuleAllHeadList,
        '/online/rule/insertTbIoTDevColRule',
        {
          dataList: this.parsingRuleAllTab.parsingRuleAllHeadList,
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          ruleColType: 'GN00000183',
          ruleMsgType: 'H' + this.parsingRuleAllTab.type,
          ruleDevType: this.devClsCd
        }
      )
    },
    ...
    saveParsingRuleAllBodyTable() {
      ...
      let duplicationCheck = this.checkDuplicateInObject(
        'ruleTargetkey',
      ...
      this.fnSearchCommon(
        this.saveParsingRuleAllBodyList,
        '/online/rule/insertTbIoTDevColRule',
        {
          dataList: this.parsingRuleAllTab.parsingRuleAllBodyList,
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          devMdlCd: this.modelConversionTable.selectDevMdlCd,
          ruleColType: 'GN00000183',
    ...
    saveControlRuleTable() {
      this.fnSearchCommon(
        this.saveControlRuleList,
        '/online/rule/insertTbIoTDevCtlRule',
        {
          dataList: this.parsingRuleCtlTab.parsingRuleCtlList,
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          ruleColType: 'GN00000209',
          ruleMsgType: this.parsingRuleAllTab.type,
          ruleDevType: this.devClsCd,
          svrMsgType: this.svrMsgType,
          devMdlCd: this.modelConversionTable.selectDevMdlCd,
    ...
    saveParsingRuleTable() {
      this.fnSearchCommon(
        this.saveParsingRuleList,
        '/online/rule/insertTbIoTDevColRule',
        {
          dataList: this.parsingRuleTab.parsingRuleList,
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          ruleColType: 'GN00000184',
          ruleDevType: this.devClsCd,
    ...
    saveVerificationRuleTable() {
      ...
      this.fnSearchCommon(
        this.saveVerificationRuleList,
        '/online/rule/insertTbIoTValRule',
        {
          dataList: this.verificationRuleTab.verificationRuleList,
          apiSeq: this.apiTable.selectApiSeq,
          devSeq: this.modelConversionTable.selectDevSeq,
          ruleMsgType: this.parsingRuleAllTab.type
        }
    ...
    search() {
      this.retrieveTabTableList()
      this.fnSearchCommon(
        this.fnAddHeadParsingRuleAllTabHeadPurposeKeyList,
        '/online/rule/retrieveIotTargetKeyParseHeaderList',
        { paramVal: this.devClsCd }
      ) // 파싱룰(전문) 헤더 목적키 리스트 조회: FIXME: 네트워크 에러
      this.fnSearchCommon(
        this.fnAddTypeList,
        '/online/rule/retrieveIotBodyType',
        {
          parentCdId: 'CM00000000',
          devClsCd: this.devClsCd,
          serverGubun: 'CM00000002'
        }
      ) // 파싱룰(전문) Type 목록 조회
      ...
      if (this.divisionCd === 'SD00000002') {
        ...
        this.fnSearchCommon(
          this.fnAddModelList,
          '/online/iotdev/retrieveIotDev',
          {
            svcCd: this.svcCd,
            devClsCd: this.devClsCd,
            displayRowCount: this.modelConversionTable.pageInfo.rowCount,
            currentPage: this.modelConversionTable.pageInfo.currentPage
          }
        ) // 모델 목록 테이블 조회
        this.fnSearchCommon(
          this.fnAddServerList,
          '/online/rule/retrieveIotServiceList',
          {}
        ) // 서버명 리스트 조회
        this.devApiNm = ''
      } else {
        ...
        this.fnSearchCommon(this.fnAddApiList, '/online/com/api/getIotApi', {
          apiNm: this.apiNm,
          displayRowCount: this.apiTable.pageInfo.rowCount,
          currentPage: this.apiTable.pageInfo.currentPage,
          serverGubun: this.retrieveData.divisionCd === '' ? 'GN00000051' : ''
        }) // API 테이블 조회
      }
    ...
    selectApiTable(apiSeq, apiNm) {
      ...
      this.fnSearchCommon(
        this.fnAddKeyList,
        '/online/com/api/retrieveTbIotParamCd',
        { apiSeq: this.apiTable.selectApiSeq }
      ) // KEY 리스트 조회
      this.retrieveTabTableList()
      // this.insertRowVerificationRuleTable()
    ...
    selectModelTable(
      ...
      if (this.modelConversionTable.selectDevSeq) {
        ...
        this.fnSearchCommon(
          this.fnAddDevRuleList,
          '/online/rule/devColRuleSettingInfo',
          {
            devSeq: this.modelConversionTable.selectDevSeq,
            displayRowCount: this.devConversionTable.pageInfo.rowCount,
            currentPage: this.devConversionTable.pageInfo.currentPage
          }
        ) // 단말별 룰 목록 테이블 조회
        this.fnSearchCommon(
          this.fnAddPurposeKeyList,
          '/online/rule/retrieveIotTargetKeyParseList',
          { devSeq: this.modelConversionTable.selectDevSeq }
        ) // 그외 목적키 리스트 조회
