    ...
    <div class="modal-body">
      <div class="form-row row py-1 border-1">
        <div class="form-item col-sm-6">
          ...
          <select
            v-model="roleCdId"
            class="form-control"
            @change="fnGetMenuRoleList"
          >
            <option
              v-for="role in roleList"
              :key="role.roleCdId"
              :value="role.roleCdId"
              >{{ role.roleCdNm }}</option
            >
          </select>
        </div>
      </div>
      <div class="table-wrap mt-3">
        ...
        <div class="btn-wrap text-center">
          <button @click="fnAllCheck()" type="button" class="btn btn-pink">
            {{ $t('all-check') }}
          </button>
        </div>
      </div>
  ...
  methods: {
    ...
    fnSetMenuRoleList(data) {
      ...
      dataList.forEach(menu => {
        ...
        // 부모메뉴가 있는 자식메뉴
        const findItemIndex = dataList.findIndex(item => {
    ...
    fnPushSelectedMenu(node) {
      ...
      let index = this.selectedMenus.findIndex(
        el => el.menuProgSeq === attr.menuProgSeq
      )
      if (index === -1) {
