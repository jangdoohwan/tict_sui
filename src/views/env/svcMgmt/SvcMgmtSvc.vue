<template>
  <div style="padding-top: 20px">
    <div class="flex flex-wrap justify-between mb-2">
      <div class="facility-tree-title">
        <h4 class="titH4">{{ $t('serviceMgmt') }}</h4>
      </div>
      <div>
        <button type="button" class="btn btn-pink" @click="fnRetvList()">
          {{ $t('re-search') }}
        </button>
        <button
          type="button"
    ...
    <div class="form-row">
      ...
      <div class="md-width-100 width-70 pl-2 md-p-0">
        <div class="form-row row">
          <div class="form-item col-md-6 pr-0">
            <span class="form-label w-44 lb-txt">{{ $t('division') }}</span>
            <input
              v-model="dataObj.division"
              type="text"
              class="form-control"
              disabled
              v-if="!isNew"
        ...
        <div class="form-row row">
          <div class="form-item col-md-6 pr-0">
            <span class="form-label w-44 lb-txt" v-show="svcType !== 'dev'">{{ $t('service-code') }}</span>
            <span class="form-label w-44 lb-txt" v-show="svcType === 'dev'">{{ $t('dev-type-code') }}</span>
            <input
              v-model="dataObj.cdId"
              type="text"
              class="form-control"
              disabled
              id="cdId"
          ...
          <div class="form-item col-md-6 pr-0">
            <span class="form-label w-44 lb-txt" v-show="svcType !== 'dev'">{{ $t('service-name') }}</span>
            <span class="form-label w-44 lb-txt" v-show="svcType === 'dev'">{{ $t('dev-type-name') }}</span>
            <div class="input-group">
              <input
                v-model="dataObj.cdNm"
                type="text"
                class="form-control"
                @input="fnIdInputChk()"
                :disabled="isModifyTrue"
                maxlength="16"
              />
              <button
                type="button"
          ...
          <div class="form-row row">
            ...
            <div class="form-item col-md-6">
              ...
              <span v-else class="form-file form-inline">
                ...
                <span v-else>
                  <span
                    >{{ defIcon.orgFileName }} [ {{ defIcon.fileSizeLabel }} ]
                  </span>
                  <span
                    class="form-data file-add"
                    @click="fnLogoDownload(defIcon.fileListSeq)"
                    ><a href="#">{{ $t('attach-file') }}</a></span
                  >
                  <span @click="fnFileDelete(defIcon.fileListSeq, 1)"
            ...
            <div class="form-item col-md-6">
              ...
              <span v-else class="form-file form-inline">
                ...
                <span v-else>
                  <span
                    >{{ stpIcon.orgFileName }} [ {{ stpIcon.fileSizeLabel }} ]
                  </span>
                  <span
                    class="form-data file-add"
                    @click="fnLogoDownload(stpIcon.fileListSeq)"
                    ><a href="#">{{ $t('attach-file') }}</a></span
                  >
                  <span @click="fnFileDelete(stpIcon.fileListSeq, 3)"
                    ><a href="#" class="btn ico-delete remove-field"></a
                  ></span>
                </span>
              </span>
          ...
          <div class="form-row row">
            ...
            <div class="form-item col-md-6">
              ...
              <span v-else class="form-file">
                ...
                <span v-else>
                  <span
                    >{{ errIcon.orgFileName }} [ {{ errIcon.fileSizeLabel }} ]
                  </span>
                  <span
                    class="form-data file-add"
                    @click="fnLogoDownload(errIcon.fileListSeq)"
                    ><a href="#">{{ $t('attach-file') }}</a></span
                  >
                  <span @click="fnFileDelete(errIcon.fileListSeq, 2)"
                    ><a href="#" class="btn ico-delete remove-field"></a
                  ></span>
                </span>
              </span>
            ...
            <div class="form-item col-md-6">
              ...
              <input
                ...
                accept=".jpg, .bmp, .ico, .gif, .png"
                :placeholder="filePlaceHolder"
                @change="fnFileCheck(7, $event)"
              />
              <span v-else class="form-file">
                <input
                  v-if="!isMultiIconExist"
                  type="file"
                  id="multiIcon"
                  name="multiIcon"
                  class="file-upload"
                ...
                <span v-else>
                  <span
                    >{{ multiIcon.orgFileName }} [
                    {{ multiIcon.fileSizeLabel }} ]
                  </span>
                  <span
                    class="form-data file-add"
                    @click="fnLogoDownload(multiIcon.fileListSeq)"
                    ><a href="#">{{ $t('attach-file') }}</a></span
                  >
                  <span @click="fnFileDelete(multiIcon.fileListSeq, 7)"
                    ><a href="#" class="btn ico-delete remove-field"></a
                  ></span>
                </span>
              </span>
          ...
          <div class="form-row row">
            ...
            <div class="form-item col-md-6">
              ...
              <input
                ...
                accept=".jpg, .bmp, .ico, .gif, .png"
                :placeholder="filePlaceHolder"
                @change="fnFileCheck(8, $event)"
              />
              <span v-else class="form-file">
                <input
                  v-if="!isMultiErrIconExist"
                  type="file"
                  id="multiErrIcon"
                  name="multiErrIcon"
                  class="file-upload"
                ...
                <span v-else>
                  <span
                    >{{ multiErrIcon.orgFileName }} [
                    {{ multiErrIcon.fileSizeLabel }} ]
                  </span>
                  <span
                    class="form-data file-add"
                    @click="fnLogoDownload(multiErrIcon.fileListSeq)"
                    ><a href="#">{{ $t('attach-file') }}</a></span
                  >
                  <span @click="fnFileDelete(multiErrIcon.fileListSeq, 8)"
                    ><a href="#" class="btn ico-delete remove-field"></a
                  ></span>
                </span>
              </span>
            ...
            <div class="form-item col-md-6">
              ...
              <input
                ...
                accept=".jpg, .bmp, .ico, .gif, .png"
                :placeholder="filePlaceHolder"
                @change="fnFileCheck(4, $event)"
              />
              <span v-else class="form-file">
                <input
                  v-if="!isGrpDefIconExist"
                  type="file"
                  id="grpDefIcon"
                  name="grpDefIcon"
                  class="file-upload"
                ...
                <span v-else>
                  <span
                    >{{ grpDefIcon.orgFileName }} [
                    {{ grpDefIcon.fileSizeLabel }} ]
                  </span>
                  <span
                    class="form-data file-add"
                    @click="fnLogoDownload(grpDefIcon.fileListSeq)"
                    ><a href="#">{{ $t('attach-file') }}</a></span
                  >
                  <span @click="fnFileDelete(grpDefIcon.fileListSeq, 4)"
                    ><a href="#" class="btn ico-delete remove-field"></a
                  ></span>
                </span>
              </span>
          ...
          <div class="form-row row">
            ...
            <div class="form-item col-md-6">
              ...
              <input
                ...
                accept=".jpg, .bmp, .ico, .gif, .png"
                :placeholder="filePlaceHolder"
                @change="fnFileCheck(5, $event)"
              />
              <span v-else class="form-file">
                <input
                  v-if="!isGrpStpIconExist"
                  type="file"
                  id="grpStpIcon"
                  name="grpStpIcon"
                  class="file-upload"
                ...
                <span v-else>
                  <span
                    >{{ grpStpIcon.orgFileName }} [
                    {{ grpStpIcon.fileSizeLabel }} ]
                  </span>
                  <span
                    class="form-data file-add"
                    @click="fnLogoDownload(grpStpIcon.fileListSeq)"
                    ><a href="#">{{ $t('attach-file') }}</a></span
                  >
                  <span @click="fnFileDelete(grpStpIcon.fileListSeq, 5)"
                    ><a href="#" class="btn ico-delete remove-field"></a
                  ></span>
                </span>
              </span>
            ...
            <div class="form-item col-md-6">
              ...
              <input
                ...
                accept=".jpg, .bmp, .ico, .gif, .png"
                :placeholder="filePlaceHolder"
                @change="fnFileCheck(6, $event)"
              />
              <span v-else class="form-file">
                <input
                  v-if="!isGrpErrIconExist"
                  type="file"
                  id="grpErrIcon"
                  name="grpErrIcon"
                  class="file-upload"
                ...
                <span v-else>
                  <span
                    >{{ grpErrIcon.orgFileName }} [
                    {{ grpErrIcon.fileSizeLabel }} ]
                  </span>
                  <span
                    class="form-data file-add"
                    @click="fnLogoDownload(grpErrIcon.fileListSeq)"
                    ><a href="#">{{ $t('attach-file') }}</a></span
                  >
                  <span @click="fnFileDelete(grpErrIcon.fileListSeq, 6)"
                    ><a href="#" class="btn ico-delete remove-field"></a
                  ></span>
                </span>
              </span>
...
export default {
  ...
  created() {
    ...
    this.fnCallAPI(
      this.retrieveSvcListAPI,
      { langSet: Cookies.get('LOCALE') },
      1
    )
  },
  mounted() {
  ...
  methods: {
      ...
      this.langSetArr.forEach(obj => {
        if (obj.cdId === this.dataObj.langSet) {
          if (this.isSaveNew) {
            ...
            if (
              this.dataObj.langSet === undefined ||
              this.dataObj.langSet === null ||
    ...
    fnFileDuplCheck() {
      var fileChkObj = {}
      /* eslint-disable */
      fileChkObj.defIconNm =
        this.defIcon.orgFileName === undefined
          ? $('#defaultIcon').val() === ''
            ? ''
            : $('#defaultIcon')
                .val()
          ...
          : this.defIcon.orgFileName
      fileChkObj.stpIconNm =
        this.stpIcon.orgFileName === undefined
          ? $('#stopIcon').val() === ''
            ? ''
            : $('#stopIcon')
                .val()
          ...
          : this.stpIcon.orgFileName
      fileChkObj.errIconNm =
        this.errIcon.orgFileName === undefined
          ? $('#errorIcon').val() === ''
            ? ''
            : $('#errorIcon')
                .val()
          ...
          : this.errIcon.orgFileName
      fileChkObj.multiIcon =
        this.multiIcon.orgFileName === undefined
          ? $('#multiIcon').val() === ''
            ? ''
            : $('#multiIcon')
                .val()
          ...
          : this.multiIcon.orgFileName
      fileChkObj.multiErrIcon =
        this.multiErrIcon.orgFileName === undefined
          ? $('#multiErrIcon').val() === ''
            ? ''
            : $('#multiErrIcon')
                .val()
          ...
          : this.multiErrIcon.orgFileName
      fileChkObj.grpDefIconNm =
        this.grpDefIcon.orgFileName === undefined
          ? $('#grpDefIcon').val() === ''
            ? ''
            : $('#grpDefIcon')
                .val()
          ...
          : this.grpDefIcon.orgFileName
      fileChkObj.grpStpIconNm =
        this.grpStpIcon.orgFileName === undefined
          ? $('#grpStpIcon').val() === ''
            ? ''
            : $('#grpStpIcon')
                .val()
          ...
          : this.grpStpIcon.orgFileName
      fileChkObj.grpErrIconNm =
        this.grpErrIcon.orgFileName === undefined
          ? $('#grpErrIcon').val() === ''
            ? ''
            : $('#grpErrIcon')
                .val()
    ...
    // 파일 업로드 시 유효성 검사
    // 1: 기본아이콘, 2: 이상아이콘, 3: 정지아이콘, 4: 그룹정상아이콘
    // 5: 그룹정지아이콘, 6: 그룹이상아이콘, 7: 멀티기본아이콘, 8: 멀티이상아이콘
    fnFileCheck(division, event) {
      // 기본아이콘
      if (division === 1) {
        ...
        if (!/\.(jpg|bmp|ico|gif|png)$/i.test(this.$refs.defaultIcon.value)) {
          if (this.$refs.defaultIcon.value !== '') {
            ...
            this.$refs.defaultIcon.value = null
          }
          // 5MB 파일 사이즈 체크
        ...
        if (!/\.(jpg|bmp|ico|gif|png)$/i.test(this.$refs.errorIcon.value)) {
          if (this.$refs.errorIcon.value !== '') {
            this.alert(this.$t('file-type-check'), 'warning')
            this.$refs.errorIcon.value = null
          }
          // 5MB 파일 사이즈 체크
        } else if (event.target.files[0].size > 5 * 1024 * 1024) {
          this.alert(this.$t('board-max-size'), 'warning')
        ...
        if (!/\.(jpg|bmp|ico|gif|png)$/i.test(this.$refs.grpDefIcon.value)) {
          if (this.$refs.grpDefIcon.value !== '') {
            this.alert(this.$t('file-type-check'), 'warning')
            this.$refs.grpDefIcon.value = null
          }
          // 5MB 파일 사이즈 체크
        } else if (event.target.files[0].size > 5 * 1024 * 1024) {
          this.alert(this.$t('board-max-size'), 'warning')
        ...
        if (!/\.(jpg|bmp|ico|gif|png)$/i.test(this.$refs.grpStpIcon.value)) {
          if (this.$refs.grpStpIcon.value !== '') {
            this.alert(this.$t('file-type-check'), 'warning')
            this.$refs.grpStpIcon.value = null
          }
          // 5MB 파일 사이즈 체크
        } else if (event.target.files[0].size > 5 * 1024 * 1024) {
          this.alert(this.$t('board-max-size'), 'warning')
        ...
        if (!/\.(jpg|bmp|ico|gif|png)$/i.test(this.$refs.grpErrIcon.value)) {
          if (this.$refs.grpErrIcon.value !== '') {
            this.alert(this.$t('file-type-check'), 'warning')
            this.$refs.grpErrIcon.value = null
          }
          // 5MB 파일 사이즈 체크
        } else if (event.target.files[0].size > 5 * 1024 * 1024) {
          this.alert(this.$t('board-max-size'), 'warning')
        ...
        if (!/\.(jpg|bmp|ico|gif|png)$/i.test(this.$refs.multiIcon.value)) {
          if (this.$refs.multiIcon.value !== '') {
            this.alert(this.$t('file-type-check'), 'warning')
            this.$refs.multiIcon.value = null
          }
          // 5MB 파일 사이즈 체크
        } else if (event.target.files[0].size > 5 * 1024 * 1024) {
          this.alert(this.$t('board-max-size'), 'warning')
        ...
        if (!/\.(jpg|bmp|ico|gif|png)$/i.test(this.$refs.multiErrIcon.value)) {
          if (this.$refs.multiErrIcon.value !== '') {
            this.alert(this.$t('file-type-check'), 'warning')
            this.$refs.multiErrIcon.value = null
          }
          // 5MB 파일 사이즈 체크
        } else if (event.target.files[0].size > 5 * 1024 * 1024) {
          this.alert(this.$t('board-max-size'), 'warning')
    ...
    fnCdDuplChk() {
      if (
        ...
        !/[ㄱ-ㅎ가-힣a-zA-Z0-9_-]{1,10}/.test('' + this.dataObj.cdId)
      ) {
    ...
    fnIdDuplChk() {
      ...
      if (
        ...
        if (this.svcType !== 'dev') {
          this.alert(this.$t('cdNm-chk'), 'warning')
        } else {
          this.alert(this.$t('devNm-chk'), 'warning')
        } 
        // 코드명에 특수문자 입력된 경우
        ...
        !/[ㄱ-ㅎ|ㅏ-ㅣ|가-힣|a-zA-Z_-]{1,16}/.test(this.dataObj.cdNm)
      ) {
        this.alert(this.$t('cdNm-spc-char-chk'), 'warning')
      } else {
        var reqData = {}
        ...
        reqData.langSet = this.dataObj.langSet
        reqData.parentCdId =
          this.svcType === 'srv' ? 'SD00000001' : 'SD00000002'
        reqData.cdSeq = this.dataObj.cdSeq
        requestApi({
    ...
    fnSvcAdd() {
      ...
      this.dataObj.division =
        this.svcType === 'srv' ? this.$t('service') : this.$t('dev-type')
      this.fnInitImgBox()
      this.fnFindTopCdId()
      this.fnGetNewCode()
      this.fnChgLang()
    ...
    submit() {
      ...
      if (!this.fnBlankCheck(this.dataObj.cdNm)) {
        ...
        !/[ㄱ-ㅎ|ㅏ-ㅣ|가-힣|a-zA-Z0-9_-]{1,16}/.test(this.dataObj.cdNm)
      ) {
        this.alert(this.$t('cdNm-spc-char-chk'), 'warning')
      } else if (!this.cdNmDuplChkFlag) {
        // 서비스 명 중복확인 하지않은 경우
        if (this.svcType !== 'dev') {
          this.alert(this.$t('cdNm-duplchk-msg'), 'warning')
        } else {
          this.alert(this.$t('devNm-duplchk-msg'), 'warning')
        }
        // 이미지 파일이 중복된 경우
    ...
    fnCallApiFile(reqData, uri, division) {
      var tmpList = []
      if (
        this.$refs.defaultIcon !== undefined &&
        this.$refs.defaultIcon.files.length > 0
      ) {
        tmpList.push(this.$refs.defaultIcon)
      }
      if (
        this.$refs.stopIcon !== undefined &&
        this.$refs.stopIcon.files.length > 0
      ) {
        tmpList.push(this.$refs.stopIcon)
      }
      if (
        this.$refs.errorIcon !== undefined &&
        this.$refs.errorIcon.files.length > 0
      ) {
        tmpList.push(this.$refs.errorIcon)
      }
      if (
        this.$refs.multiIcon !== undefined &&
        this.$refs.multiIcon.files.length > 0
      ) {
        tmpList.push(this.$refs.multiIcon)
      }
      if (
        this.$refs.multiErrIcon !== undefined &&
        this.$refs.multiErrIcon.files.length > 0
      ) {
        tmpList.push(this.$refs.multiErrIcon)
      }
      if (
        this.$refs.grpDefIcon !== undefined &&
        this.$refs.grpDefIcon.files.length > 0
      ) {
        tmpList.push(this.$refs.grpDefIcon)
      }
      if (
        this.$refs.grpErrIcon !== undefined &&
        this.$refs.grpErrIcon.files.length > 0
      ) {
        tmpList.push(this.$refs.grpErrIcon)
      }
      if (
        this.$refs.grpStpIcon !== undefined &&
        this.$refs.grpStpIcon.files.length > 0
      ) {
