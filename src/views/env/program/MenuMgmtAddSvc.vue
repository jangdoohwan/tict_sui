<template>
  <div>
    <div class="modal-body">
      <div class="form-item">
        <span class="form-label w-32 lb-txt">{{ $t('up-menunm') }}</span>
        <input v-model="parentNm" type="text" class="form-control" readonly />
      </div>
      <div class="form-item">
        <span class="form-label w-32 lb-txt">{{ $t('menu-name') }}</span>
        <input
          v-model="menuCdNm"
          type="text"
          class="form-control"
      ...
      <div class="form-item">
        <span class="form-label w-32 lb-txt">{{ $t('charSet') }}</span>
...
export default {
  ...
  methods: {
    ...
    showConfirmPopup(msg) {
      // 메뉴명 입력하지 않은 경우
      if (!this.menuCdNm || (this.menuCdNm && this.menuCdNm.trim() === '')) {
