  ...
  <div>
      ...
      <div class="form-inline">
        <div class="inline">
          ...
          <select
            v-model="depth2"
            class="form-control"
            @change="changeCommonCode(2)"
            :disabled="data.depth2"
          >
            <option value="">{{ $t('all') }}</option>
            <option
              v-for="(depth2, index) in depth2List"
              :key="index"
              :value="depth2.cdId"
              >{{ depth2.cdNm }}</option
            >
          ...
          <span class="form-label mx-1">&gt;</span>
          <select
            ref="commonSelectbox3"
            v-model="depth3"
            class="form-control"
            @change="changeCommonCode(3)"
            :disabled="data.depth3 || (depth2 ? false : true)"
          >
            <option value="">{{ $t('all') }}</option>
            <option
              v-for="(depth3, index) in depth3List"
              :key="index"
              :value="depth3.cdId"
              >{{ depth3.cdNm }}</option
            >
          ...
          <select
            v-model="depth4"
            class="form-control"
            @change="changeCommonCode(4)"
            :disabled="depth3 ? false : true"
          >
            <option value="">{{ $t('all') }}</option>
            <option
              v-for="(depth4, index) in depth4List"
              :key="index"
              :value="depth4.cdId"
              >{{ depth4.cdNm }}</option
            >
            ...
            <table class="table table-col">
              ...
              <thead>
                <tr>
                  ...
                  <th scope="col">
                    <div class="fixed-th">{{ $t('cdNm') }}</div>
                  </th>
                  <th scope="col">
                    <div class="fixed-th">{{ $t('code') }}</div>
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr
                  v-for="commonCode in commonCodeList"
                  :key="commonCode.cdSeq"
                >
                  <td>
                    <div class="form-check">
                      <label class="form-check-label">
                        <input
                          class="form-check-input eleCheck"
                          type="checkbox"
                          @change="
                            changeCheckbox($event.target.checked, commonCode)
                          "
                        />
                        <span class="form-check-sign"></span>
                      </label>
    ...
    <div class="modal-footer custom-footer">
      <div class="btn-wrap text-center">
        <button @click="submitDevice()" type="button" class="btn btn-pink">
          {{ $t('confirm') }}
        </button>
        <button
          @click="close()"
  ...
  methods: {
    ...
    changeCheckbox(checked, commonCode) {
      if (checked) {
        ...
        this.selectedCommonCode = this.selectedCommonCode.filter(
          selectedCommonCode => selectedCommonCode.cdSeq !== commonCode.cdSeq
        )
      }
    },
    submitDevice() {
      ...
      if (
        this.data &&
        !this.data.isMultiSelect &&
        this.selectedCommonCode.length > 1
      ) {
      ...
      if (this.data && !this.data.isParentSelect) {
        ...
        if (selectedParent) {
          this.alert(
            this.$t('alert-dont-select-parent-cmcd').replace(
              '$0',
              selectedParent
            )
          )
