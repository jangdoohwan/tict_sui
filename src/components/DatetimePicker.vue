    ...
    <template v-if="isPair">
      <div ref="startDateWrap" class="input-group startDate">
        <input
          ...
          :style="`width: ${width}`"
        />
        <span class="input-group-addon cal-bg"
          ><span class="date-icon"></span
        ></span>
      </div>
      <span class="mx-0" style="padding-left: 2px; padding-right: 7px">~</span>
...
export default {
  ...
  props: {
    isPair: {
      // true 설정 시 검색기간 시작,종료일 모두 표시
      type: Boolean,
      required: true
    },
    month: {
      // true 설정 시 'YYYY-MM' 형식으로 보여짐
      type: Boolean,
      required: false,
      default: false
    },
    hour: {
      // true 설정 시 'YYYY-MM-DD HH:00' 형식으로 보여짐
      type: Boolean,
      required: false,
      default: false
    },
    minute: {
      // true 설정 시 'YYYY-MM-DD HH:mm' 형식으로 보여짐 (hour가 true 되어있어야함)
      type: Boolean,
      required: false,
      default: false
    },
    second: {
      // true 설정 시 'YYYY-MM-DD HH:mm:ss' 형식으로 보여짐 (hour, minute 가 true 되어있어야함)
      type: Boolean,
      required: false,
      default: false
    },
    setSideBySide: {
      // 단일 Date 에서만 사용가능하며 sideBySide 형식으로 보여진다.
      type: Boolean,
      required: false,
      default: false
    },
  ...
  watch: {
    date: function (newValue) {
      if (newValue instanceof Date) {
        $(this.$refs.dateWrap)
          .data('DateTimePicker')
          .date(this.getFormatedDatetime(newValue))
      } else if (typeof newValue === 'string') {
        $(this.$refs.dateWrap).data('DateTimePicker').date(new Date(newValue))
      } else {
    ...
    startDate: function (newValue) {
      if (this.isStartDateInnerUpdate) {
        ...
        let startDatetimePicker = $(this.$refs.startDateWrap).data(
          'DateTimePicker'
        )
        if (newValue instanceof Date) {
    ...
    if (this.isPair) {
      ...
      if (this.date) {
        if (this.date instanceof Date) {
          $(this.$refs.dateWrap).data('DateTimePicker').date(this.date)
        } else if (typeof this.date === 'string' && this.date.length > 0) {
          $(this.$refs.dateWrap)
            .data('DateTimePicker')
  ...
  methods: {
    ...
    setStartDatetimePicker(endDate) {
      ...
      let maxDate =
        this.maxDate !== 'now'
          ? this.getFormatedDatetime(this.maxDate)
      ...
      let maxDuration =
        endDate &&
        this.duration &&
        (this.duration.days || this.duration.months || this.duration.hours) // endDate 가 있으며 duration 값이 있는경우
          ? this.duration.days
            ...
            : moment(endDate).subtract(this.duration.hours, 'hours') // duration.hours 있는경우
      ...
      let date =
        maxDuration && maxDuration.isAfter(startDateTimePicker.date())
          ? maxDuration
    ...
    setEndDatetimePicker(startDate) {
      ...
      let maxDuration =
        startDate &&
        this.duration &&
        (this.duration.days || this.duration.months || this.duration.hours) // startDate 가 있으며 duration 값이 있는경우
          ? this.duration.days
            ...
            : moment(startDate).add(this.duration.hours, 'hours') // duration.hours 있는경우
      ...
      // 이후날짜 비활성화 셋팅
      let max = maxDate !== null ? maxDate : false // maxDate 가 있으면 maxDate 셋팅, 없으면 false(기본값)

      let date =
        maxDuration && maxDuration.isBefore(endDateTimePicker.date())
          ? maxDuration
