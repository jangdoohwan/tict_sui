  ...
  methods: {
    getValues() {
      ...
      this.sortedMsgData.forEach(d => {
        let data = this.componentData.find(
          c => String(c.entrDevSeq) === String(d.ENTR_DEV)
        )
        if (data && data.dataObj) {
          let dataObj = JSON.parse(data.dataObj)
          let det = dataObj.find(i => i['COL_NM_CD'] === d['TAG_VALUE_BY_CLS'])
          this.values.push(!det || det['DET_STATUS_CD_ID'] === '')
        } else {
