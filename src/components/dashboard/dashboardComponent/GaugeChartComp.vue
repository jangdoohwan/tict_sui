<template>
  <div :id="gaugeChart + locNo" :style="{ width: width, height: height }"></div>
</template>
<script>
// initMsg Example
...
export default {
  ...
  watch: {
    componentData: function (componentData) {
      for (let index = 0; index < this.initMsgData.length; ++index) {
        ...
        this.options.series[index].data[0].value = parseFloat(
          componentData[tagValue]
        ).toFixed(3)
      }
  ...
  mounted() {
    ...
    this.myChart = echarts.init(
      document.getElementById(this.gaugeChart + this.locNo)
    )
    this.myChart.setOption(this.options, true)
    ...
    init() {
      ...
      for (let index = 0; index < this.initMsgData.length; ++index) {
        this.options.series[index].data = [
          { value: 0, name: this.initMsgData[index].TITLE }
        ]
        this.options.series[index].min = parseFloat(
          this.initMsgData[index].MIN
        ).toFixed(3)
        this.options.series[index].max = parseFloat(
          this.initMsgData[index].MAX
        ).toFixed(3)
        ...
        if (width >= height) {
          ...
          // 반지름 계산
          this.options.series[index].radius =
            Math.min((width / height) * 50, 100) + '%'
        } else {
          // 위치계산
