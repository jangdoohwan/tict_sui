  ...
  <div class="pagination-wrap">
    <ul class="pagination">
      ...
      <li
        class="pg-ico prev"
        :class="isFirstPage"
        @click="!isFirstPage && setPage(currentPage - 1)"
      >
        <a>이전</a>
      </li>
      <li class="pagination-box">
        <p>페이지</p>
        <input
          class="pagination-input"
          type="text"
          v-model="currentPage"
          @keyup.enter="setPage(currentPage)"
        />
        <p>
          /
        </p>
      ...
      <li
        class="pg-ico next"
        :class="isLastPage"
        @click="!isLastPage && setPage(currentPage + 1)"
      >
        <a>다음</a>
      </li>
      <li
        class="pg-ico end"
        :class="isLastPage"
        @click="!isLastPage && setPage(totalPage)"
      >
        <a>마지막</a>
      </li>
    </ul>
    ...
    >
      <option
        v-for="(value, key, index) in rowCountArray"
        :key="index"
        :value="value"
        role="option"
        >{{ value }}</option
...
export default {
  ...
  computed: {
    ...
    isLastPage() {
      return this.selectedPage === this.totalPage || this.totalPage <= 1
        ? 'disabled'
        : ''
    },
    getVisiblePageArray() {
      ...
      if (this.selectedPage <= 3) {
        ...
        this.totalPage >= 4 &&
        this.totalPage - 2 < this.selectedPage
      ) {
        for (
          let a = this.totalPage - (this.totalPage === 4 ? 3 : 4);
          a <= this.totalPage;
          a++
        ) {
          visiblePageArray.push(a)
  ...
  watch: {
    totalPage(val) {
      // totalPage 값이 변경되면 pageList 재 셋팅 해줘야 함
      this.setVisiblePage()
    }
  },
  mounted() {
